...
namespace jsonifier {
template <> struct core<twitter_full::search_metadata_data> {
...
template <> struct core<twitter_full::media_data> {
...
template <> struct core<twitter_full::url_data> {
...
template <> struct core<twitter_full::user_mention_data> {
...
template <> struct core<twitter_full::status_entities> {
...
template <> struct core<twitter_full::status_data> {
...
template <> struct core<twitter_full::twitter_message> {
...
struct jsonifier_ondemand {
  ...
  bool run(simdjson::padded_string &json,
    ...
    std::string_view json_view(json.data(), json.size());

    parser.parseJson<jsonifier::parse_options{.knownOrder = true}>(

        result.message, json_view);

    return result.message.statuses.size() > 0;
  }
};
BENCHMARK_TEMPLATE(twitter_full, jsonifier_ondemand)->UseManualTime();
} // namespace top_tweet
