...
struct simdjson2_ondemand {
  ...
  bool run(simdjson2::padded_string &json, int64_t max_retweet_count, top_tweet_result<StringType> &result) {
    ...
    // NOTE: simdjson2 does not presently support reuse of objects or arrays--just scalars. This is
...
BENCHMARK_TEMPLATE(top_tweet, simdjson2_ondemand)->UseManualTime();

struct simdjson2_ondemand_forward_only {
  using StringType=std::string_view;
...
BENCHMARK_TEMPLATE(top_tweet, simdjson2_ondemand_forward_only)->UseManualTime();
