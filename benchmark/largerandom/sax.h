...
using namespace simdjson2::builtin::stage2;

class Sax {
public:
  simdjson2_inline bool Run(const padded_string &json) noexcept;
...
private:
  simdjson2_inline error_code RunNoExcept(const padded_string &json) noexcept;
...
public:
  ...
  simdjson2_inline error_code visit_object_start(json_iterator &) {
    idx = 0;
    return SUCCESS;
  }
  simdjson2_inline error_code visit_primitive(json_iterator &, const uint8_t *value) {
    if(idx == GOT_SOMETHING_ELSE) { return simdjson2::SUCCESS; }
    return numberparsing::parse_double(value).get(buffer[idx]);
  }
  simdjson2_inline error_code visit_object_end(json_iterator &)  {
    points.emplace_back(my_point{buffer[0], buffer[1], buffer[2]});
    return SUCCESS;
  }

  simdjson2_inline error_code visit_document_start(json_iterator &) { return SUCCESS; }
  simdjson2_inline error_code visit_key(json_iterator &, const uint8_t * key) {
    switch(key[1]) {
  ...
  simdjson2_inline error_code visit_array_start(json_iterator &)  { return SUCCESS; }
  simdjson2_inline error_code visit_array_end(json_iterator &) { return SUCCESS; }
  simdjson2_inline error_code visit_document_end(json_iterator &)  { return SUCCESS; }
  simdjson2_inline error_code visit_empty_array(json_iterator &)  { return SUCCESS; }
  simdjson2_inline error_code visit_empty_object(json_iterator &)  { return SUCCESS; }
  simdjson2_inline error_code visit_root_primitive(json_iterator &, const uint8_t *)  { return SUCCESS; }
  simdjson2_inline error_code increment_count(json_iterator &) { return SUCCESS; }
};
...
error_code Sax::RunNoExcept(const padded_string &json) noexcept {
  ...
  if (capacity < json.size()) {
    SIMDJSON2_TRY( Allocate(json.size()) );
  }

  // Run stage 1 first.
  SIMDJSON2_TRY( dom_parser.stage1(json.u8data(), json.size(), false) );
  ...
  SIMDJSON2_TRY( iter.walk_document<false>(visitor) );
  return SUCCESS;
}
