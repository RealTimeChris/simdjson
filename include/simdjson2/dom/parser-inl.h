#ifndef SIMDJSON2_PARSER_INL_H
#define SIMDJSON2_PARSER_INL_H

#include "simdjson2/dom/base.h"
#include "simdjson2/dom/document_stream.h"
...
#include "simdjson2/dom/document_stream-inl.h"
#include "simdjson2/dom/element-inl.h"
...
//
simdjson2_inline parser::parser(size_t max_capacity) noexcept
  : _max_capacity{max_capacity},
    loaded_bytes(nullptr) {
}
simdjson2_inline parser::parser(parser &&other) noexcept = default;
simdjson2_inline parser &parser::operator=(parser &&other) noexcept = default;
...
inline simdjson2_result<size_t> parser::read_file(const std::string &path) noexcept {
  // Open the file
  ...
  // Get the file size
  int ret;
#if SIMDJSON2_VISUAL_STUDIO && !SIMDJSON2_IS_32BITS
...
inline simdjson2_result<element> parser::load(const std::string &path) & noexcept {
  return load_into_document(doc, path);
}

inline simdjson2_result<element> parser::load_into_document(document& provided_doc, const std::string &path) & noexcept {
  size_t len;
...
inline simdjson2_result<document_stream> parser::load_many(const std::string &path, size_t batch_size) noexcept {
  size_t len;
  auto _error = read_file(path).get(len);
  if (_error) { return _error; }
...
inline simdjson2_result<element> parser::parse_into_document(document& provided_doc, const uint8_t *buf, size_t len, bool realloc_if_needed) & noexcept {
...
simdjson2_inline simdjson2_result<element> parser::parse_into_document(document& provided_doc, const char *buf, size_t len, bool realloc_if_needed) & noexcept {
  return parse_into_document(provided_doc, reinterpret_cast<const uint8_t *>(buf), len, realloc_if_needed);
}
simdjson2_inline simdjson2_result<element> parser::parse_into_document(document& provided_doc, const std::string &s) & noexcept {
  return parse_into_document(provided_doc, s.data(), s.length(), s.capacity() - s.length() < SIMDJSON2_PADDING);
}
simdjson2_inline simdjson2_result<element> parser::parse_into_document(document& provided_doc, const padded_string &s) & noexcept {
  return parse_into_document(provided_doc, s.data(), s.length(), false);
}


inline simdjson2_result<element> parser::parse(const uint8_t *buf, size_t len, bool realloc_if_needed) & noexcept {
  return parse_into_document(doc, buf, len, realloc_if_needed);
}

simdjson2_inline simdjson2_result<element> parser::parse(const char *buf, size_t len, bool realloc_if_needed) & noexcept {
  return parse(reinterpret_cast<const uint8_t *>(buf), len, realloc_if_needed);
}
simdjson2_inline simdjson2_result<element> parser::parse(const std::string &s) & noexcept {
  return parse(s.data(), s.length(), s.capacity() - s.length() < SIMDJSON2_PADDING);
}
simdjson2_inline simdjson2_result<element> parser::parse(const padded_string &s) & noexcept {
  return parse(s.data(), s.length(), false);
}
simdjson2_inline simdjson2_result<element> parser::parse(const padded_string_view &v) & noexcept {
  return parse(v.data(), v.length(), false);
}

inline simdjson2_result<document_stream> parser::parse_many(const uint8_t *buf, size_t len, size_t batch_size) noexcept {
  if(batch_size < MINIMAL_BATCH_SIZE) { batch_size = MINIMAL_BATCH_SIZE; }
  if((len >= 3) && (std::memcmp(buf, "\xEF\xBB\xBF", 3) == 0)) {
    buf += 3;
    len -= 3;
...
inline simdjson2_result<document_stream> parser::parse_many(const char *buf, size_t len, size_t batch_size) noexcept {
  return parse_many(reinterpret_cast<const uint8_t *>(buf), len, batch_size);
}
inline simdjson2_result<document_stream> parser::parse_many(const std::string &s, size_t batch_size) noexcept {
  return parse_many(s.data(), s.length(), batch_size);
}
inline simdjson2_result<document_stream> parser::parse_many(const padded_string &s, size_t batch_size) noexcept {
  return parse_many(s.data(), s.length(), batch_size);
}

simdjson2_inline size_t parser::capacity() const noexcept {
  return implementation ? implementation->capacity() : 0;
}
simdjson2_inline size_t parser::max_capacity() const noexcept {
  return _max_capacity;
}
simdjson2_pure simdjson2_inline size_t parser::max_depth() const noexcept {
...
inline error_code parser::allocate(size_t capacity, size_t max_depth) noexcept {
  ...
  if (implementation) {
    ...
    err = simdjson2::get_active_implementation()->create_dom_parser_implementation(capacity, max_depth, implementation);
  }
  if (err) { return err; }
  return SUCCESS;
}
...
inline error_code parser::ensure_capacity(document& target_document, size_t desired_capacity) noexcept {
  ...
  if (simdjson2_unlikely(capacity() < desired_capacity || target_document.capacity() < desired_capacity)) {
...
simdjson2_inline void parser::set_max_capacity(size_t max_capacity) noexcept {
