#ifndef SIMDJSON2_DOCUMENT_STREAM_INL_H
#define SIMDJSON2_DOCUMENT_STREAM_INL_H

#include "simdjson2/dom/base.h"
#include "simdjson2/dom/document_stream.h"
#include "simdjson2/dom/element-inl.h"
#include "simdjson2/dom/parser-inl.h"
...
simdjson2_inline document_stream::document_stream(
  dom::parser &_parser,
  const uint8_t *_buf,
  size_t _len,
  size_t _batch_size
...
simdjson2_inline document_stream::document_stream() noexcept
  : parser{nullptr},
    buf{nullptr},
    len{0},
    batch_size{0},
...
simdjson2_inline document_stream::iterator::iterator() noexcept
  : stream{nullptr}, finished{true} {
}

simdjson2_inline document_stream::iterator document_stream::begin() noexcept {
  start();
  // If there are no documents, we're finished.
  return iterator(this, error == EMPTY);
}

simdjson2_inline document_stream::iterator document_stream::end() noexcept {
  return iterator(this, true);
}

simdjson2_inline document_stream::iterator::iterator(document_stream* _stream, bool is_end) noexcept
  : stream{_stream}, finished{is_end} {
}

simdjson2_inline document_stream::iterator::reference document_stream::iterator::operator*() noexcept {
...
simdjson2_inline document_stream::iterator& document_stream::iterator::operator++() noexcept {
...
simdjson2_inline bool document_stream::iterator::operator!=(const document_stream::iterator &other) const noexcept {
...
#endif // SIMDJSON2_THREADS_ENABLED
  next();
}

simdjson2_inline size_t document_stream::iterator::current_index() const noexcept {
  return stream->doc_index;
}

simdjson2_inline std::string_view document_stream::iterator::source() const noexcept {
...
simdjson2_inline simdjson2_result<dom::document_stream>::simdjson2_result() noexcept
  : simdjson2_result_base() {
}
simdjson2_inline simdjson2_result<dom::document_stream>::simdjson2_result(error_code error) noexcept
  : simdjson2_result_base(error) {
}
simdjson2_inline simdjson2_result<dom::document_stream>::simdjson2_result(dom::document_stream &&value) noexcept
  : simdjson2_result_base(std::forward<dom::document_stream>(value)) {
}

#if SIMDJSON2_EXCEPTIONS
simdjson2_inline dom::document_stream::iterator simdjson2_result<dom::document_stream>::begin() noexcept(false) {
  if (error()) { throw simdjson2_error(error()); }
  return first.begin();
}
simdjson2_inline dom::document_stream::iterator simdjson2_result<dom::document_stream>::end() noexcept(false) {
  if (error()) { throw simdjson2_error(error()); }
  return first.end();
}
#else // SIMDJSON2_EXCEPTIONS
#ifndef SIMDJSON2_DISABLE_DEPRECATED_API
simdjson2_inline dom::document_stream::iterator simdjson2_result<dom::document_stream>::begin() noexcept {
  first.error = error();
  return first.begin();
}
simdjson2_inline dom::document_stream::iterator simdjson2_result<dom::document_stream>::end() noexcept {
  first.error = error();
  return first.end();
}
...
#endif // SIMDJSON2_DOCUMENT_STREAM_INL_H
