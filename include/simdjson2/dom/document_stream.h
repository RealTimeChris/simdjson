#ifndef SIMDJSON2_DOCUMENT_STREAM_H
#define SIMDJSON2_DOCUMENT_STREAM_H
...
public:
  ...
  class iterator {
  public:
    using value_type = simdjson2_result<element>;
...
private:
   ...
   *
   * The function returns simdjson2::EMPTY if there is no more data to be parsed.
   *
   * The function returns simdjson2::SUCCESS (as integer = 0) in case of success
   * and indicates that the buffer has successfully been parsed to the end.
   * Every document it contained has been parsed without error.
   *
   * The function returns an error code from simdjson2/simdjson2.h in case of failure
   * such as simdjson2::CAPACITY, simdjson2::MEMALLOC, simdjson2::DEPTH_ERROR and so forth;
   * the simdjson2::error_message function converts these error codes into a string).
   *
  ...
  friend struct simdjson2_result<dom::document_stream>;
  friend struct internal::simdjson2_result_base<dom::document_stream>;
...
template<>
struct simdjson2_result<dom::document_stream> : public internal::simdjson2_result_base<dom::document_stream> {
public:
  simdjson2_inline simdjson2_result() noexcept; ///< @private
  simdjson2_inline simdjson2_result(error_code error) noexcept; ///< @private
  simdjson2_inline simdjson2_result(dom::document_stream &&value) noexcept; ///< @private

#if SIMDJSON2_EXCEPTIONS
  simdjson2_inline dom::document_stream::iterator begin() noexcept(false);
  simdjson2_inline dom::document_stream::iterator end() noexcept(false);
  ...
  simdjson2_inline dom::document_stream::iterator begin() noexcept;
  [[deprecated("parse_many() and load_many() may return errors. Use document_stream stream; error = parser.parse_many().get(doc); instead.")]]
  simdjson2_inline dom::document_stream::iterator end() noexcept;
#endif // SIMDJSON2_DISABLE_DEPRECATED_API
#endif // SIMDJSON2_EXCEPTIONS
}; // struct simdjson2_result<dom::document_stream>

} // namespace simdjson2

#endif // SIMDJSON2_DOCUMENT_STREAM_H
