#ifndef SIMDJSON2_DOM_PARSER_H
#define SIMDJSON2_DOM_PARSER_H
...
public:
   ...
   *    Defaults to SIMDJSON2_MAXSIZE_BYTES (the largest single document simdjson2 can process).
   */
  simdjson2_inline explicit parser(size_t max_capacity = SIMDJSON2_MAXSIZE_BYTES) noexcept;
  /**
   * Take another parser's buffers and state.
   *
   * @param other The parser to take. Its capacity is zeroed.
   */
  simdjson2_inline parser(parser &&other) noexcept;
   ...
   * @param other The parser to take. Its capacity is zeroed.
   */
  simdjson2_inline parser &operator=(parser &&other) noexcept;
   ...
   */
  inline simdjson2_result<element> load(const std::string &path) & noexcept;
  inline simdjson2_result<element> load(const std::string &path) &&  = delete ;
   ...
   *           same document: they may vary under runtime dispatch (so they may vary depending on your system and hardware).
   */
  inline simdjson2_result<element> load_into_document(document& doc, const std::string &path) & noexcept;
  inline simdjson2_result<element> load_into_document(document& doc, const std::string &path) && =delete;
   ...
   *
   * The buffer must have at least SIMDJSON2_PADDING extra allocated bytes. It does not matter what
   ...
   * SIMDJSON2_PADDING bytes to avoid runtime warnings.
   *
   ...
   * you must provide a buffer with at least SIMDJSON2_PADDING extra bytes at the end.
   ...
   * to handle it (up to max_capacity).
   *
   * @param buf The JSON to parse. Must have at least len + SIMDJSON2_PADDING allocated bytes, unless
   ...
   *           same document: they may vary under runtime dispatch (so they may vary depending on your system and hardware).
   */
  inline simdjson2_result<element> parse(const uint8_t *buf, size_t len, bool realloc_if_needed = true) & noexcept;
  inline simdjson2_result<element> parse(const uint8_t *buf, size_t len, bool realloc_if_needed = true) && =delete;
  /** @overload parse(const uint8_t *buf, size_t len, bool realloc_if_needed) */
  simdjson2_inline simdjson2_result<element> parse(const char *buf, size_t len, bool realloc_if_needed = true) & noexcept;
  simdjson2_inline simdjson2_result<element> parse(const char *buf, size_t len, bool realloc_if_needed = true) && =delete;
  /** @overload parse(const uint8_t *buf, size_t len, bool realloc_if_needed) */
  simdjson2_inline simdjson2_result<element> parse(const std::string &s) & noexcept;
  simdjson2_inline simdjson2_result<element> parse(const std::string &s) && =delete;
  /** @overload parse(const uint8_t *buf, size_t len, bool realloc_if_needed) */
  simdjson2_inline simdjson2_result<element> parse(const padded_string &s) & noexcept;
  simdjson2_inline simdjson2_result<element> parse(const padded_string &s) && =delete;
  /** @overload parse(const uint8_t *buf, size_t len, bool realloc_if_needed) */
  simdjson2_inline simdjson2_result<element> parse(const padded_string_view &v) & noexcept;
  simdjson2_inline simdjson2_result<element> parse(const padded_string_view &v) && =delete;

  /** @private We do not want to allow implicit conversion from C string to std::string. */
  simdjson2_inline simdjson2_result<element> parse(const char *buf) noexcept = delete;
   ...
   *           same document: they may vary under runtime dispatch (so they may vary depending on your system and hardware).
   */
  inline simdjson2_result<element> parse_into_document(document& doc, const uint8_t *buf, size_t len, bool realloc_if_needed = true) & noexcept;
  inline simdjson2_result<element> parse_into_document(document& doc, const uint8_t *buf, size_t len, bool realloc_if_needed = true) && =delete;
  /** @overload parse_into_document(const uint8_t *buf, size_t len, bool realloc_if_needed) */
  simdjson2_inline simdjson2_result<element> parse_into_document(document& doc, const char *buf, size_t len, bool realloc_if_needed = true) & noexcept;
  simdjson2_inline simdjson2_result<element> parse_into_document(document& doc, const char *buf, size_t len, bool realloc_if_needed = true) && =delete;
  /** @overload parse_into_document(const uint8_t *buf, size_t len, bool realloc_if_needed) */
  simdjson2_inline simdjson2_result<element> parse_into_document(document& doc, const std::string &s) & noexcept;
  simdjson2_inline simdjson2_result<element> parse_into_document(document& doc, const std::string &s) && =delete;
  /** @overload parse_into_document(const uint8_t *buf, size_t len, bool realloc_if_needed) */
  simdjson2_inline simdjson2_result<element> parse_into_document(document& doc, const padded_string &s) & noexcept;
  simdjson2_inline simdjson2_result<element> parse_into_document(document& doc, const padded_string &s) && =delete;

  /** @private We do not want to allow implicit conversion from C string to std::string. */
  simdjson2_inline simdjson2_result<element> parse_into_document(document& doc, const char *buf) noexcept = delete;
   ...
   *
   * As with all other simdjson2 methods, non-exception error handling is readily available through
   ...
   * ### Threads
   *
   * When compiled with SIMDJSON2_THREADS_ENABLED, this method will use a single thread under the
   ...
   *                   Defaults to 1MB (as simdjson2::dom::DEFAULT_BATCH_SIZE), which has been a reasonable sweet
   *                   spot in our tests.
   *                   If you set the batch_size to a value smaller than simdjson2::dom::MINIMAL_BATCH_SIZE
   *                   (currently 32B), it will be replaced by simdjson2::dom::MINIMAL_BATCH_SIZE.
   ...
   *           same document: they may vary under runtime dispatch (so they may vary depending on your system and hardware).
   */
  inline simdjson2_result<document_stream> load_many(const std::string &path, size_t batch_size = dom::DEFAULT_BATCH_SIZE) noexcept;
   ...
   * to handle it (up to max_capacity).
   *
   * @param buf The concatenated JSON to parse. Must have at least len + SIMDJSON2_PADDING allocated bytes.
   ...
   *           same document: they may vary under runtime dispatch (so they may vary depending on your system and hardware).
   */
  inline simdjson2_result<document_stream> parse_many(const uint8_t *buf, size_t len, size_t batch_size = dom::DEFAULT_BATCH_SIZE) noexcept;
  /** @overload parse_many(const uint8_t *buf, size_t len, size_t batch_size) */
  inline simdjson2_result<document_stream> parse_many(const char *buf, size_t len, size_t batch_size = dom::DEFAULT_BATCH_SIZE) noexcept;
  /** @overload parse_many(const uint8_t *buf, size_t len, size_t batch_size) */
  inline simdjson2_result<document_stream> parse_many(const std::string &s, size_t batch_size = dom::DEFAULT_BATCH_SIZE) noexcept;
  inline simdjson2_result<document_stream> parse_many(const std::string &&s, size_t batch_size) = delete;// unsafe
  /** @overload parse_many(const uint8_t *buf, size_t len, size_t batch_size) */
  inline simdjson2_result<document_stream> parse_many(const padded_string &s, size_t batch_size = dom::DEFAULT_BATCH_SIZE) noexcept;
  inline simdjson2_result<document_stream> parse_many(const padded_string &&s, size_t batch_size) = delete;// unsafe

  /** @private We do not want to allow implicit conversion from C string to std::string. */
  simdjson2_result<document_stream> parse_many(const char *buf, size_t batch_size = dom::DEFAULT_BATCH_SIZE) noexcept = delete;
   ...
   */
  simdjson2_warn_unused inline error_code allocate(size_t capacity, size_t max_depth = DEFAULT_MAX_DEPTH) noexcept;
  ...
  simdjson2_warn_unused inline bool allocate_capacity(size_t capacity, size_t max_depth = DEFAULT_MAX_DEPTH) noexcept;
   ...
   */
  simdjson2_inline size_t capacity() const noexcept;
   ...
   */
  simdjson2_inline size_t max_capacity() const noexcept;
   ...
   */
  simdjson2_pure simdjson2_inline size_t max_depth() const noexcept;
   ...
   */
  simdjson2_inline void set_max_capacity(size_t max_capacity) noexcept;
...
#else
  /**
   * When SIMDJSON2_THREADS_ENABLED is not defined, the parser instance cannot use threads.
   */
  bool threaded{false};
#endif
  /** @private Use the new DOM API instead */
  class Iterator;
  /** @private Use simdjson2_error instead */
  using InvalidJSON [[deprecated("Use simdjson2_error instead")]] = simdjson2_error;
   ...
   * simdjson2.h will return UNINITIALIZED if no parsing was attempted
   */
  [[deprecated("Use the result of parser.parse() instead")]]
...
private:
  ...
  inline simdjson2_result<size_t> read_file(const std::string &path) noexcept;
