#ifndef SIMDJSON2_SERIALIZATION_H
#define SIMDJSON2_SERIALIZATION_H
...
public:
  /** Add a comma **/
  simdjson2_inline void comma();
  /** Start an array, prints [ **/
  simdjson2_inline void start_array();
  /** End an array, prints ] **/
  simdjson2_inline void end_array();
  /** Start an array, prints { **/
  simdjson2_inline void start_object();
  /** Start an array, prints } **/
  simdjson2_inline void end_object();
  /** Prints a true **/
  simdjson2_inline void true_atom();
  /** Prints a false **/
  simdjson2_inline void false_atom();
  /** Prints a null **/
  simdjson2_inline void null_atom();
  /** Prints a number **/
  simdjson2_inline void number(int64_t x);
  /** Prints a number **/
  simdjson2_inline void number(uint64_t x);
  /** Prints a number **/
  simdjson2_inline void number(double x);
  /** Prints a key (string + colon) **/
  simdjson2_inline void key(std::string_view unescaped);
  /** Prints a string. The string is escaped as needed. **/
  simdjson2_inline void string(std::string_view unescaped);
  /** Clears out the content. **/
  simdjson2_inline void clear();
  /**
   * Get access to the buffer, it is owned by the instance, but
   * the user can make a copy.
   **/
  simdjson2_inline std::string_view str() const;

  /** Prints one character **/
  simdjson2_inline void one_char(char c);

  simdjson2_inline void call_print_newline() {
      static_cast<formatter*>(this)->print_newline();
  }

  simdjson2_inline void call_print_indents(size_t depth) {
      static_cast<formatter*>(this)->print_indents(depth);
  }

  simdjson2_inline void call_print_space() {
...
public:
  simdjson2_inline void print_newline();

  simdjson2_inline void print_indents(size_t depth);

  simdjson2_inline void print_space();
};
...
public:
  ...
  inline void append(simdjson2::dom::element value);
  /** Append an array to the builder (to be printed) **/
  inline void append(simdjson2::dom::array value);
  /** Append an object to the builder (to be printed) **/
  inline void append(simdjson2::dom::object value);
  ...
  simdjson2_inline void append(simdjson2::dom::key_value_pair value);
private:
  formatter format{};
};
...
#if SIMDJSON2_EXCEPTIONS
template <class T>
std::string to_string(simdjson2_result<T> x) {
    if (x.error()) { throw simdjson2_error(x.error()); }
...
#if SIMDJSON2_EXCEPTIONS
template <class T>
std::string minify(simdjson2_result<T> x) {
    if (x.error()) { throw simdjson2_error(x.error()); }
    return to_string(x.value());
}
#endif
...
std::string prettify(T x)  {
    simdjson2::internal::string_builder<simdjson2::internal::pretty_formatter> sb;
    sb.append(x);
    std::string_view answer = sb.str();
    return std::string(answer.data(), answer.size());
}
...
#if SIMDJSON2_EXCEPTIONS
template <class T>
std::string prettify(simdjson2_result<T> x) {
    if (x.error()) { throw simdjson2_error(x.error()); }
    return to_string(x.value());
}
#endif
