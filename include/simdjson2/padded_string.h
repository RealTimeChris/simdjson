#ifndef SIMDJSON2_PADDED_STRING_H
#define SIMDJSON2_PADDED_STRING_H
...
struct padded_string final {
  ...
  const uint8_t *u8data() const noexcept {
    return static_cast<const uint8_t *>(static_cast<const void *>(data_ptr));
  }
   ...
   **/
  inline static simdjson2_result<padded_string>
 ...
 * @throw if there is an error with the underlying output stream. simdjson2
 * itself will not throw.
 */
inline std::ostream &operator<<(std::ostream &out, const padded_string &s) {
  return out << s.data();
}
 ...
 * @param out The output stream.
 * @param s The padded_string instance.
 * @throw simdjson2_error if the result being printed has an error. If there is
 * an error with the underlying output stream, that error will be propagated
 * (simdjson2_error will not be thrown).
 */
inline std::ostream &
operator<<(std::ostream &out,
           simdjson2_result<padded_string> &s) noexcept(false) {
  return out << s.value();
}
#endif
...
#endif // SIMDJSON2_PADDED_STRING_H
