#ifndef SIMDJSON2_INTERNAL_DOM_PARSER_IMPLEMENTATION_H
#define SIMDJSON2_INTERNAL_DOM_PARSER_IMPLEMENTATION_H
...
/**
 * An implementation of simdjson2's DOM parser for a particular CPU architecture.
 *
...
public:
   ...
   *
   * @param buf The json document to parse. *MUST* be allocated up to len + SIMDJSON2_PADDING bytes.
   * @param len The length of the json document.
   * @return The error code, or SUCCESS if there was no error.
   */
  simdjson2_warn_unused virtual error_code parse(const uint8_t *buf, size_t len, dom::document &doc) noexcept = 0;
   ...
   * @return The error code, or SUCCESS if there was no error.
   */
  simdjson2_warn_unused virtual error_code stage1(const uint8_t *buf, size_t len, stage1_mode streaming) noexcept = 0;
   ...
   * @return The error code, or SUCCESS if there was no error.
   */
  simdjson2_warn_unused virtual error_code stage2(dom::document &doc) noexcept = 0;
   ...
   */
  simdjson2_warn_unused virtual error_code stage2_next(dom::document &doc) noexcept = 0;
   ...
   */
  simdjson2_warn_unused virtual uint8_t *parse_string(const uint8_t *src, uint8_t *dst, bool allow_replacement) const noexcept = 0;
   ...
   * @return end of the of the written region (exclusive) or nullptr in case of error.
   */
  simdjson2_warn_unused virtual uint8_t *parse_wobbly_string(const uint8_t *src, uint8_t *dst) const noexcept = 0;

  /**
   * Change the capacity of this parser.
   *
   * The capacity can never exceed SIMDJSON2_MAXSIZE_BYTES (e.g., 4 GB)
   ...
   */
  simdjson2_warn_unused inline error_code allocate(size_t capacity, size_t max_depth) noexcept;
...
protected:
  ...
  simdjson2_inline dom_parser_implementation() noexcept;
  simdjson2_inline dom_parser_implementation(dom_parser_implementation &&other) noexcept;
  simdjson2_inline dom_parser_implementation &operator=(dom_parser_implementation &&other) noexcept;

  simdjson2_inline dom_parser_implementation(const dom_parser_implementation &) noexcept = delete;
  simdjson2_inline dom_parser_implementation &operator=(const dom_parser_implementation &other) noexcept = delete;
}; // class dom_parser_implementation

simdjson2_inline dom_parser_implementation::dom_parser_implementation() noexcept = default;
simdjson2_inline dom_parser_implementation::dom_parser_implementation(dom_parser_implementation &&other) noexcept = default;
simdjson2_inline dom_parser_implementation &dom_parser_implementation::operator=(dom_parser_implementation &&other) noexcept = default;

simdjson2_pure simdjson2_inline size_t dom_parser_implementation::capacity() const noexcept {
  return _capacity;
}

simdjson2_pure simdjson2_inline size_t dom_parser_implementation::max_depth() const noexcept {
  return _max_depth;
}
...
#endif // SIMDJSON2_INTERNAL_DOM_PARSER_IMPLEMENTATION_H
