#ifndef SIMDJSON2_ARM64_BITMANIPULATION_H
#define SIMDJSON2_ARM64_BITMANIPULATION_H
...
// but the algorithms do not end up using the returned value.
// Sadly, sanitizers are not smart enough to figure it out.
// Applies only when SIMDJSON2_PREFER_REVERSE_BITS is defined and true.
// (See below.)
SIMDJSON2_NO_SANITIZE_UNDEFINED
/* result might be undefined when input_num is zero */
...
/* result might be undefined when input_num is zero */
simdjson2_inline int count_ones(uint64_t input_num) {
 ...
 */
/*
 * We use SIMDJSON2_PREFER_REVERSE_BITS as a hint that algorithms that
 * work well with bit reversal may use it.
 */
#define SIMDJSON2_PREFER_REVERSE_BITS 1

/* reverse the bits */
simdjson2_inline uint64_t reverse_bits(uint64_t input_num) {
  uint64_t rev_bits;
 ...
 **/
SIMDJSON2_NO_SANITIZE_UNDEFINED
simdjson2_inline uint64_t zero_leading_bit(uint64_t rev_bits, int leading_zeroes) {
  return rev_bits ^ (uint64_t(0x8000000000000000) >> leading_zeroes);
}

#endif

simdjson2_inline bool add_overflow(uint64_t value1, uint64_t value2, uint64_t *result) {
#ifdef SIMDJSON2_REGULAR_VISUAL_STUDIO
...
#endif // SIMDJSON2_ARM64_BITMANIPULATION_H
