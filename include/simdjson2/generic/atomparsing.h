#ifndef SIMDJSON2_GENERIC_ATOMPARSING_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_ATOMPARSING_H
...
simdjson2_really_inline uint32_t string_to_uint32(const char* str) { uint32_t val; std::memcpy(&val, str, sizeof(uint32_t)); return val; }
...
simdjson2_really_inline uint32_t str4ncmp(const uint8_t *src, const char* atom) {
  uint32_t srcval; // we want to avoid unaligned 32-bit loads (undefined in C/C++)
  static_assert(sizeof(uint32_t) <= SIMDJSON2_PADDING, "SIMDJSON2_PADDING must be larger than 4 bytes");
  std::memcpy(&srcval, src, sizeof(uint32_t));
  return srcval ^ string_to_uint32(atom);
}

simdjson2_warn_unused
simdjson2_really_inline bool is_valid_true_atom(const uint8_t *src) {
  return (str4ncmp(src, "true") | jsoncharutils::is_not_structural_or_whitespace(src[4])) == 0;
}

simdjson2_warn_unused
simdjson2_really_inline bool is_valid_true_atom(const uint8_t *src, size_t len) {
...
simdjson2_warn_unused
simdjson2_really_inline bool is_valid_false_atom(const uint8_t *src) {
  return (str4ncmp(src+1, "alse") | jsoncharutils::is_not_structural_or_whitespace(src[5])) == 0;
}

simdjson2_warn_unused
simdjson2_really_inline bool is_valid_false_atom(const uint8_t *src, size_t len) {
...
simdjson2_warn_unused
simdjson2_really_inline bool is_valid_null_atom(const uint8_t *src) {
  return (str4ncmp(src, "null") | jsoncharutils::is_not_structural_or_whitespace(src[4])) == 0;
}

simdjson2_warn_unused
simdjson2_really_inline bool is_valid_null_atom(const uint8_t *src, size_t len) {
...
#endif // SIMDJSON2_GENERIC_ATOMPARSING_H
