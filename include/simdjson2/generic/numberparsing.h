#ifndef SIMDJSON2_GENERIC_NUMBERPARSING_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_NUMBERPARSING_H
...
simdjson2_inline double to_double(uint64_t mantissa, uint64_t real_exponent, bool negative) {
    double d;
    mantissa &= ~(1ULL << 52);
...
simdjson2_inline bool compute_float_64(int64_t power, uint64_t i, bool negative, double &d) {
...
#error "FLT_EVAL_METHOD should be defined, please include cfloat."
...
#endif
  {
    ...
    //
    if (power < 0) {
      d = d / simdjson2::internal::power_of_ten[-power];
    } else {
      d = d * simdjson2::internal::power_of_ten[power];
    }
    if (negative) {
  ...
  // 1.
  const uint32_t index = 2 * uint32_t(power - simdjson2::internal::smallest_power);
  ...
  simdjson2::internal::value128 firstproduct = full_multiplication(i, simdjson2::internal::power_of_five_128[index]);
  ...
  if((firstproduct.high & 0x1FF) == 0x1FF) {
    ...
    // 64-bit least significant bits of the product and with a "high component" corresponding
    // to the 64-bit most significant bits of the product.
    simdjson2::internal::value128 secondproduct = full_multiplication(i, simdjson2::internal::power_of_five_128[index + 1]);
  ...
  if (simdjson2_unlikely(real_exponent <= 0)) { // we have a subnormal?
  ...
  if (simdjson2_unlikely((lower <= 1) && (power >= -4) && (power <= 23) && ((mantissa & 3) == 1))) {
  ...
  if (simdjson2_unlikely(real_exponent > 2046)) {
...
static bool parse_float_fallback(const uint8_t *ptr, double *outDouble) {
  *outDouble = simdjson2::internal::from_chars(reinterpret_cast<const char *>(ptr));
  ...
  // https://github.com/simdjson2/simdjson2/issues/1286
  //
...
static bool parse_float_fallback(const uint8_t *ptr, const uint8_t *end_ptr, double *outDouble) {
  *outDouble = simdjson2::internal::from_chars(reinterpret_cast<const char *>(ptr), reinterpret_cast<const char *>(end_ptr));
  // We do not accept infinite values.
...
simdjson2_inline bool is_made_of_eight_digits_fast(const uint8_t *chars) {
  uint64_t val;
  // this can read up to 7 bytes beyond the buffer size, but we require
  // SIMDJSON2_PADDING of padding
  static_assert(7 <= SIMDJSON2_PADDING, "SIMDJSON2_PADDING must be bigger than 7");
...
template<typename I>
SIMDJSON2_NO_SANITIZE_UNDEFINED // We deliberately allow overflow here and check later
simdjson2_inline bool parse_digit(const uint8_t c, I &i) {
...
simdjson2_inline bool is_digit(const uint8_t c) {
  return static_cast<uint8_t>(c - '0') <= 9;
}

simdjson2_inline error_code parse_decimal_after_separator(simdjson2_unused const uint8_t *const src, const uint8_t *&p, uint64_t &i, int64_t &exponent) {
...
#ifdef SIMDJSON2_SWAR_NUMBER_PARSING
#if SIMDJSON2_SWAR_NUMBER_PARSING
...
#endif // SIMDJSON2_SWAR_NUMBER_PARSING
#endif // #ifdef SIMDJSON2_SWAR_NUMBER_PARSING
...
simdjson2_inline error_code parse_exponent(simdjson2_unused const uint8_t *const src, const uint8_t *&p, int64_t &exponent) {
  ...
  // Performance notes: it may seem like combining the two "simdjson2_unlikely checks" below into
  // a single simdjson2_unlikely path would be faster. The reasoning is sound, but the compiler may
  ...
  // instructions for a simdjson2_likely branch, an unconclusive gain.

  // If there were no digits, it's an error.
  if (simdjson2_unlikely(p == start_exp)) {
    return INVALID_NUMBER(src);
  }
  ...
  // something!!!!
  if (simdjson2_unlikely(p > start_exp+18)) {
...
simdjson2_inline bool check_if_integer(const uint8_t *const src, size_t max_length) {
...
simdjson2_inline size_t significant_digits(const uint8_t * start_digits, size_t digit_count) {
...
/** @private */
static error_code slow_float_parsing(simdjson2_unused const uint8_t * src, double* answer) {
...
/** @private */
template<typename W>
simdjson2_inline error_code write_float(const uint8_t *const src, bool negative, uint64_t i, const uint8_t * start_digits, size_t digit_count, int64_t exponent, W &writer) {
  ...
  if (simdjson2_unlikely(digit_count > 19 && significant_digits(start_digits, digit_count) > 19)) {
  ...
  // NOTE: it's weird that the simdjson2_unlikely() only wraps half the if, but it seems to get slower any other
  // way we've tried: https://github.com/simdjson2/simdjson2/pull/990#discussion_r448497331
  // To future reader: we'd love if someone found a better way, or at least could explain this result!
  if (simdjson2_unlikely(exponent < simdjson2::internal::smallest_power) || (exponent > simdjson2::internal::largest_power)) {
    //
    ...
    static_assert(simdjson2::internal::smallest_power <= -342, "smallest_power is not small enough");
    //
    if((exponent < simdjson2::internal::smallest_power) || (i == 0)) {
      // E.g. Parse "-0.0e-999" into the same value as "-0.0". See https://en.wikipedia.org/wiki/Signed_zero
      WRITE_DOUBLE(negative ? -0.0 : 0.0, src, writer);
      return SUCCESS;
    } else { // (exponent > largest_power) and (i != 0)
      // We have, for sure, an infinite value and simdjson2 refuses to parse infinite values.
      return INVALID_NUMBER(src);
    }
  }
  double d;
...
template<typename W>
simdjson2_inline error_code parse_number(const uint8_t *const src, W &writer);

// for performance analysis, it is sometimes  useful to skip parsing
#ifdef SIMDJSON2_SKIPNUMBERPARSING

template<typename W>
simdjson2_inline error_code parse_number(const uint8_t *const, W &writer) {
  writer.append_s64(0);        // always write zero
  return SUCCESS;              // always succeeds
}

simdjson2_unused simdjson2_inline simdjson2_result<uint64_t> parse_unsigned(const uint8_t * const src) noexcept { return 0; }
simdjson2_unused simdjson2_inline simdjson2_result<int64_t> parse_integer(const uint8_t * const src) noexcept { return 0; }
simdjson2_unused simdjson2_inline simdjson2_result<double> parse_double(const uint8_t * const src) noexcept { return 0; }
simdjson2_unused simdjson2_inline simdjson2_result<uint64_t> parse_unsigned_in_string(const uint8_t * const src) noexcept { return 0; }
simdjson2_unused simdjson2_inline simdjson2_result<int64_t> parse_integer_in_string(const uint8_t * const src) noexcept { return 0; }
simdjson2_unused simdjson2_inline simdjson2_result<double> parse_double_in_string(const uint8_t * const src) noexcept { return 0; }
simdjson2_unused simdjson2_inline bool is_negative(const uint8_t * src) noexcept  { return false; }
simdjson2_unused simdjson2_inline simdjson2_result<bool> is_integer(const uint8_t * src) noexcept  { return false; }
simdjson2_unused simdjson2_inline simdjson2_result<number_type> get_number_type(const uint8_t * src) noexcept { return number_type::signed_integer; }
#else
...
simdjson2_inline error_code parse_number(const uint8_t *const src, W &writer) {
  ...
  if ('.' == *p) {
    is_float = true;
    ++p;
    SIMDJSON2_TRY( parse_decimal_after_separator(src, p, i, exponent) );
    digit_count = int(p - start_digits); // used later to guard against overflows
  }
  if (('e' == *p) || ('E' == *p)) {
    is_float = true;
    ++p;
    SIMDJSON2_TRY( parse_exponent(src, p, exponent) );
  }
  if (is_float) {
    const bool dirty_end = jsoncharutils::is_not_structural_or_whitespace(*p);
    SIMDJSON2_TRY( write_float(src, negative, i, start_digits, digit_count, exponent, writer) );
...
simdjson2_unused simdjson2_inline simdjson2_result<uint64_t> parse_unsigned(const uint8_t * const src) noexcept {
  const uint8_t *p = src;
  //
  // Parse the integer part.
  //
...
simdjson2_unused simdjson2_inline simdjson2_result<uint64_t> parse_unsigned(const uint8_t * const src, const uint8_t * const src_end) noexcept {
  const uint8_t *p = src;
  //
  // Parse the integer part.
  //
...
// Parse any number from 0 to 18,446,744,073,709,551,615
simdjson2_unused simdjson2_inline simdjson2_result<uint64_t> parse_unsigned_in_string(const uint8_t * const src) noexcept {
...
simdjson2_unused simdjson2_inline simdjson2_result<int64_t> parse_integer(const uint8_t *src) noexcept {
  //
  // Check for minus sign
  //
  bool negative = (*src == '-');
...
// Parse any number from  -9,223,372,036,854,775,808 to 9,223,372,036,854,775,807
// Never read at src_end or beyond
simdjson2_unused simdjson2_inline simdjson2_result<int64_t> parse_integer(const uint8_t * const src, const uint8_t * const src_end) noexcept {
  //
  // Check for minus sign
  //
...
// Parse any number from  -9,223,372,036,854,775,808 to 9,223,372,036,854,775,807
simdjson2_unused simdjson2_inline simdjson2_result<int64_t> parse_integer_in_string(const uint8_t *src) noexcept {
  //
  // Check for minus sign
  //
  bool negative = (*(src + 1) == '-');
...
simdjson2_unused simdjson2_inline simdjson2_result<double> parse_double(const uint8_t * src) noexcept {
  //
  // Check for minus sign
  //
  bool negative = (*src == '-');
  ...
  if (simdjson2_likely(*p == '.')) {
    ...
    overflow = p-src-1 > 19;
    if (simdjson2_unlikely(overflow && leading_zero)) {
  ...
  overflow = overflow || exponent < simdjson2::internal::smallest_power || exponent > simdjson2::internal::largest_power;

  //
  // Assemble (or slow-parse) the float
  //
  double d;
  if (simdjson2_likely(!overflow)) {
...
simdjson2_unused simdjson2_inline bool is_negative(const uint8_t * src) noexcept {
  return (*src == '-');
}

simdjson2_unused simdjson2_inline simdjson2_result<bool> is_integer(const uint8_t * src) noexcept {
  bool negative = (*src == '-');
  src += uint8_t(negative);
  const uint8_t *p = src;
...
simdjson2_unused simdjson2_inline simdjson2_result<number_type> get_number_type(const uint8_t * src) noexcept {
  bool negative = (*src == '-');
  src += uint8_t(negative);
  const uint8_t *p = src;
  while(static_cast<uint8_t>(*p - '0') <= 9) { p++; }
  ...
  if (jsoncharutils::is_structural_or_whitespace(*p)) {
    ...
    // We have an integer.
    if(simdjson2_unlikely(digit_count > 20)) {
      return number_type::big_integer;
    }
    // If the number is negative and valid, it must be a signed integer.
    if(negative) {
      if (simdjson2_unlikely(digit_count > 19)) return number_type::big_integer;
      if (simdjson2_unlikely(digit_count == 19 && memcmp(src, smaller_big_integer, 19) > 0)) {
        return number_type::big_integer;
      }
...
// Never read at src_end or beyond
simdjson2_unused simdjson2_inline simdjson2_result<double> parse_double(const uint8_t * src, const uint8_t * const src_end) noexcept {
  if(src == src_end) { return NUMBER_ERROR; }
  //
  // Check for minus sign
  //
  ...
  int64_t exponent = 0;
  bool overflow;
  if (simdjson2_likely((p != src_end) && (*p == '.'))) {
    p++;
    const uint8_t *start_decimal_digits = p;
...
simdjson2_unused simdjson2_inline simdjson2_result<double> parse_double_in_string(const uint8_t * src) noexcept {
  //
  // Check for minus sign
  //
  bool negative = (*(src + 1) == '-');
...
#endif // SIMDJSON2_SKIPNUMBERPARSING
...
#endif // SIMDJSON2_GENERIC_NUMBERPARSING_H
