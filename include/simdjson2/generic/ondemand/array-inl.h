#ifndef SIMDJSON2_GENERIC_ONDEMAND_ARRAY_INL_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_ONDEMAND_ARRAY_INL_H
...
simdjson2_inline array::array(const value_iterator &_iter) noexcept
  : iter{_iter}
{
}

simdjson2_inline simdjson2_result<array> array::start(value_iterator &iter) noexcept {
  // We don't need to know if the array is empty to start iteration, but we do want to know if there
  // is an error--thus `simdjson2_unused`.
  simdjson2_unused bool has_value;
  SIMDJSON2_TRY( iter.start_array().get(has_value) );
  return array(iter);
}
simdjson2_inline simdjson2_result<array> array::start_root(value_iterator &iter) noexcept {
  simdjson2_unused bool has_value;
  SIMDJSON2_TRY( iter.start_root_array().get(has_value) );
  return array(iter);
}
simdjson2_inline simdjson2_result<array> array::started(value_iterator &iter) noexcept {
  bool has_value;
  SIMDJSON2_TRY(iter.started_array().get(has_value));
  return array(iter);
}

simdjson2_inline simdjson2_result<array_iterator> array::begin() noexcept {
#if SIMDJSON2_DEVELOPMENT_CHECKS
  if (!iter.is_at_iterator_start()) { return OUT_OF_ORDER_ITERATION; }
#endif
  return array_iterator(iter);
}
simdjson2_inline simdjson2_result<array_iterator> array::end() noexcept {
  return array_iterator(iter);
}
simdjson2_inline error_code array::consume() noexcept {
  auto error = iter.json_iter().skip_child(iter.depth()-1);
  if(error) { iter.abandon(); }
  return error;
}

simdjson2_inline simdjson2_result<std::string_view> array::raw_json() noexcept {
...
simdjson2_inline simdjson2_result<size_t> array::count_elements() & noexcept {
  size_t count{0};
...
simdjson2_inline simdjson2_result<bool> array::is_empty() & noexcept {
  bool is_not_empty;
...
inline simdjson2_result<bool> array::reset() & noexcept {
  return iter.reset_array();
}

inline simdjson2_result<value> array::at_pointer(std::string_view json_pointer) noexcept {
...
inline simdjson2_result<value> array::at_path(std::string_view json_path) noexcept {
  auto json_pointer = json_path_to_pointer_conversion(json_path);
  if (json_pointer == "-1") { return INVALID_JSON_POINTER; }
  return at_pointer(json_pointer);
}

simdjson2_inline simdjson2_result<value> array::at(size_t index) noexcept {
  size_t i = 0;
  for (auto value : *this) {
...
simdjson2_inline simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::simdjson2_result(
  SIMDJSON2_IMPLEMENTATION::ondemand::array &&value
) noexcept
  : implementation_simdjson2_result_base<SIMDJSON2_IMPLEMENTATION::ondemand::array>(
      std::forward<SIMDJSON2_IMPLEMENTATION::ondemand::array>(value)
    )
{
}
simdjson2_inline simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::simdjson2_result(
  ...
  : implementation_simdjson2_result_base<SIMDJSON2_IMPLEMENTATION::ondemand::array>(error)
{
}

simdjson2_inline simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array_iterator> simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::begin() noexcept {
  if (error()) { return error(); }
  return first.begin();
}
simdjson2_inline simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array_iterator> simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::end() noexcept {
  if (error()) { return error(); }
  return first.end();
}
simdjson2_inline  simdjson2_result<size_t> simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::count_elements() & noexcept {
  if (error()) { return error(); }
  return first.count_elements();
}
simdjson2_inline  simdjson2_result<bool> simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::is_empty() & noexcept {
  if (error()) { return error(); }
  return first.is_empty();
}
simdjson2_inline  simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::value> simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::at(size_t index) noexcept {
  if (error()) { return error(); }
  return first.at(index);
}
simdjson2_inline  simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::value> simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::at_pointer(std::string_view json_pointer) noexcept {
  if (error()) { return error(); }
  return first.at_pointer(json_pointer);
}
simdjson2_inline  simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::value> simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::at_path(std::string_view json_path) noexcept {
  if (error()) { return error(); }
  return first.at_path(json_path);
}
simdjson2_inline  simdjson2_result<std::string_view> simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::array>::raw_json() noexcept {
  if (error()) { return error(); }
  return first.raw_json();
}
} // namespace simdjson2

#endif // SIMDJSON2_GENERIC_ONDEMAND_ARRAY_INL_H
