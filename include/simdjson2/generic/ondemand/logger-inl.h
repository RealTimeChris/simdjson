#ifndef SIMDJSON2_GENERIC_ONDEMAND_LOGGER_INL_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_ONDEMAND_LOGGER_INL_H
...
static inline log_level get_log_level_from_env()
{
  ...
  SIMDJSON2_DISABLE_DEPRECATED_WARNING // Disable CRT_SECURE warning on MSVC: manually verified this is safe
      char *lvl = getenv("SIMDJSON2_LOG_LEVEL");
  SIMDJSON2_POP_DISABLE_WARNINGS
  if (lvl && simdjson2_strcasecmp(lvl, "ERROR") == 0) { return log_level::error; }
...
inline void log_headers() noexcept {
  if (LOG_ENABLED) {
    if (simdjson2_unlikely(should_log(log_level::info))) {
...
inline void log_line(const json_iterator &iter, token_position index, depth_t depth, const char *title_prefix, const char *title, std::string_view detail, log_level level, Args&&... args) noexcept {
  if (LOG_ENABLED) {
    if (simdjson2_unlikely(should_log(level))) {
             ...
             LOG_EVENT_LEN - indent - int(strlen(title_prefix)), msg.c_str());
      {
        ...
        // This could be unsafe, see https://github.com/simdjson2/simdjson2/discussions/1938
        if (index < iter._root) {
...
#endif // SIMDJSON2_GENERIC_ONDEMAND_LOGGER_INL_H