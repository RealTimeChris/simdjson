#ifndef SIMDJSON2_GENERIC_ONDEMAND_JSON_ITERATOR_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_ONDEMAND_JSON_ITERATOR_H
...
public:
  simdjson2_inline json_iterator() noexcept = default;
  simdjson2_inline json_iterator(json_iterator &&other) noexcept;
  simdjson2_inline json_iterator &operator=(json_iterator &&other) noexcept;
  simdjson2_inline explicit json_iterator(const json_iterator &other) noexcept = default;
  simdjson2_inline json_iterator &operator=(const json_iterator &other) noexcept = default;
  /**
   * Skips a JSON value, whether it is a scalar, array or object.
   */
  simdjson2_warn_unused simdjson2_inline error_code skip_child(depth_t parent_depth) noexcept;

  /**
   * Tell whether the iterator is still at the start
   */
  simdjson2_inline bool at_root() const noexcept;
   ...
   */
  simdjson2_inline bool streaming() const noexcept;

  /**
   * Get the root value iterator
   */
  simdjson2_inline token_position root_position() const noexcept;
  /**
   * Assert that we are at the document depth (== 1)
   */
  simdjson2_inline void assert_at_document_depth() const noexcept;
  /**
   * Assert that we are at the root of the document
   */
  simdjson2_inline void assert_at_root() const noexcept;
   ...
   */
  simdjson2_inline bool is_alive() const noexcept;
   ...
   */
  simdjson2_inline bool is_single_token() const noexcept;
   ...
   */
  simdjson2_inline void assert_more_tokens(uint32_t required_tokens=1) const noexcept;
  /**
   * Assert that the given position addresses an actual token (is within bounds).
   *
   * Has no effect in release builds.
   */
  simdjson2_inline void assert_valid_position(token_position position) const noexcept;
  /**
   ...
   */
  simdjson2_inline const uint8_t *unsafe_pointer() const noexcept;
  /**
   ...
   * it is not used ...
   */
  simdjson2_inline const uint8_t *peek_last() const noexcept;
   ...
   */
  simdjson2_inline void ascend_to(depth_t parent_depth) noexcept;
   ...
   */
  simdjson2_inline void descend_to(depth_t child_depth) noexcept;
  simdjson2_inline void descend_to(depth_t child_depth, int32_t delta) noexcept;

  /**
   * Get current depth.
   */
  simdjson2_inline depth_t depth() const noexcept;
   ...
   * @param message An error message to report with the error.
   */
  simdjson2_inline error_code optional_error(error_code error, const char *message) noexcept;
   ...
   */
  simdjson2_warn_unused simdjson2_inline bool copy_to_buffer(const uint8_t *json, uint32_t max_len, uint8_t *tmpbuf, size_t N) noexcept;
   ...
   * overflow.
   */
  simdjson2_inline simdjson2_result<std::string_view> unescape(raw_json_string in, bool allow_replacement) noexcept;
  simdjson2_inline simdjson2_result<std::string_view> unescape_wobbly(raw_json_string in) noexcept;

  simdjson2_inline void reenter_child(token_position position, depth_t child_depth) noexcept;

  simdjson2_inline error_code consume_character(char c) noexcept;
#if SIMDJSON2_DEVELOPMENT_CHECKS
  simdjson2_inline token_position start_position(depth_t depth) const noexcept;
  simdjson2_inline void set_start_position(depth_t depth, token_position position) noexcept;
#endif
...
protected:
  simdjson2_inline json_iterator(const uint8_t *buf, ondemand::parser *parser) noexcept;
#ifdef SIMDJSON2_EXPERIMENTAL_ALLOW_INCOMPLETE_JSON
  simdjson2_inline json_iterator(const uint8_t *buf, ondemand::parser *parser, bool streaming) noexcept;
  ...
  simdjson2_inline token_position end() const noexcept;
...
template<>
struct simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::json_iterator> : public SIMDJSON2_IMPLEMENTATION::implementation_simdjson2_result_base<SIMDJSON2_IMPLEMENTATION::ondemand::json_iterator> {
public:
  simdjson2_inline simdjson2_result(SIMDJSON2_IMPLEMENTATION::ondemand::json_iterator &&value) noexcept; ///< @private
...
#endif // SIMDJSON2_GENERIC_ONDEMAND_JSON_ITERATOR_H