#ifndef SIMDJSON2_GENERIC_ONDEMAND_VALUE_ITERATOR_INL_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_ONDEMAND_VALUE_ITERATOR_INL_H
...
simdjson2_inline value_iterator::value_iterator(
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::start_object() noexcept {
  SIMDJSON2_TRY( start_container('{', "Not an object", "object") );
  return started_object();
}

simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::start_root_object() noexcept {
  SIMDJSON2_TRY( start_container('{', "Not an object", "object") );
  return started_root_object();
}

simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::started_object() noexcept {
...
simdjson2_warn_unused simdjson2_inline error_code value_iterator::check_root_object() noexcept {
  ...
  if ( ! _json_iter->streaming() ) {
    ...
    // https://github.com/simdjson2/simdjson2/issues/1834
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::started_root_object() noexcept {
  auto error = check_root_object();
  if(error) { return error; }
  return started_object();
}

simdjson2_warn_unused simdjson2_inline error_code value_iterator::end_container() noexcept {
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::has_next_field() noexcept {
  assert_at_next();
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::find_field_raw(const std::string_view key) noexcept {
  error_code error;
  bool has_value;
  //
  ...
  while (has_value) {
    ...
    SIMDJSON2_TRY( skip_child() ); // Skip the value entirely
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::find_field_unordered_raw(const std::string_view key) noexcept {
  /**
  ...
  //
    ...
    if (_json_iter->depth() < depth() - 1) { return OUT_OF_ORDER_ITERATION; }
#endif
    SIMDJSON2_TRY(reset_object().get(has_value));
    at_first = true;
  // 3. When a previous search found a field or an iterator yielded a value:
  //
  //    ```
  ...
  while (has_value) {
    SIMDJSON2_ASSUME( _json_iter->_depth == _depth ); // We must be at the start of a field
  ...
  SIMDJSON2_TRY(reset_object().get(has_value));
  while (true) {
    SIMDJSON2_ASSUME(has_value); // we should reach search_start before ever reaching the end of the object
    SIMDJSON2_ASSUME( _json_iter->_depth == _depth ); // We must be at the start of a field
    ...
    // field_key() advances the pointer and checks that '"' is found (corresponding to a key).
    // The depth is left unchanged by field_key().
    error = field_key().get(actual_key); SIMDJSON2_ASSUME(!error);
    // field_value() will advance and check that we find a ':' separating the
    // key and the value.  It will also increment the depth by one.
    error = field_value(); SIMDJSON2_ASSUME(!error);
    ...
    // It returns true if ',' is found, false otherwise. If anything other than ',' or '}' is found,
    // then we are in error and we abort.
    error = has_next_field().get(has_value); SIMDJSON2_ASSUME(!error);
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<raw_json_string> value_iterator::field_key() noexcept {
  assert_at_next();
...
simdjson2_warn_unused simdjson2_inline error_code value_iterator::field_value() noexcept {
  assert_at_next();
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::start_array() noexcept {
  SIMDJSON2_TRY( start_container('[', "Not an array", "array") );
  return started_array();
}

simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::start_root_array() noexcept {
  SIMDJSON2_TRY( start_container('[', "Not an array", "array") );
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::started_array() noexcept {
  assert_at_container_start();
  if (*_json_iter->peek() == ']') {
...
simdjson2_warn_unused simdjson2_inline error_code value_iterator::check_root_array() noexcept {
  // When in streaming mode, we cannot expect peek_last() to be the last structural element of the
  // current document. It only works in the normal mode where we have indexed a single document.
  // Note that adding a check for 'streaming' is not expensive since we only have at most
  // one root element.
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::started_root_array() noexcept {
  auto error = check_root_array();
  if (error) { return error; }
  return started_array();
}

simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::has_next_element() noexcept {
  assert_at_next();
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::parse_bool(const uint8_t *json) const noexcept {
  ...
  return simdjson2_result<bool>(!not_true);
}
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::parse_null(const uint8_t *json) const noexcept {
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<std::string_view> value_iterator::get_string(bool allow_replacement) noexcept {
  return get_raw_json_string().unescape(json_iter(), allow_replacement);
}
template <typename string_type>
simdjson2_warn_unused simdjson2_inline error_code value_iterator::get_string(string_type& receiver, bool allow_replacement) noexcept {
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<std::string_view> value_iterator::get_wobbly_string() noexcept {
  return get_raw_json_string().unescape_wobbly(json_iter());
}
simdjson2_warn_unused simdjson2_inline simdjson2_result<raw_json_string> value_iterator::get_raw_json_string() noexcept {
  auto json = peek_scalar("string");
  if (*json != '"') { return incorrect_type_error("Not a string"); }
  advance_scalar("string");
  return raw_json_string(json+1);
}
simdjson2_warn_unused simdjson2_inline simdjson2_result<uint64_t> value_iterator::get_uint64() noexcept {
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<uint64_t> value_iterator::get_uint64_in_string() noexcept {
  auto result = numberparsing::parse_unsigned_in_string(peek_non_root_scalar("uint64"));
  if(result.error() == SUCCESS) { advance_non_root_scalar("uint64"); }
  return result;
}
simdjson2_warn_unused simdjson2_inline simdjson2_result<int64_t> value_iterator::get_int64() noexcept {
  auto result = numberparsing::parse_integer(peek_non_root_scalar("int64"));
  if(result.error() == SUCCESS) { advance_non_root_scalar("int64"); }
  return result;
}
simdjson2_warn_unused simdjson2_inline simdjson2_result<int64_t> value_iterator::get_int64_in_string() noexcept {
  auto result = numberparsing::parse_integer_in_string(peek_non_root_scalar("int64"));
  if(result.error() == SUCCESS) { advance_non_root_scalar("int64"); }
  return result;
}
simdjson2_warn_unused simdjson2_inline simdjson2_result<double> value_iterator::get_double() noexcept {
  auto result = numberparsing::parse_double(peek_non_root_scalar("double"));
  if(result.error() == SUCCESS) { advance_non_root_scalar("double"); }
  return result;
}
simdjson2_warn_unused simdjson2_inline simdjson2_result<double> value_iterator::get_double_in_string() noexcept {
  auto result = numberparsing::parse_double_in_string(peek_non_root_scalar("double"));
  if(result.error() == SUCCESS) { advance_non_root_scalar("double"); }
  return result;
}
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::get_bool() noexcept {
  auto result = parse_bool(peek_non_root_scalar("bool"));
  if(result.error() == SUCCESS) { advance_non_root_scalar("bool"); }
  return result;
}
simdjson2_inline simdjson2_result<bool> value_iterator::is_null() noexcept {
  bool is_null_value;
  SIMDJSON2_TRY(parse_null(peek_non_root_scalar("null")).get(is_null_value));
  if(is_null_value) { advance_non_root_scalar("null"); }
  return is_null_value;
}
simdjson2_inline bool value_iterator::is_negative() noexcept {
  return numberparsing::is_negative(peek_non_root_scalar("numbersign"));
}
simdjson2_inline bool value_iterator::is_root_negative() noexcept {
  return numberparsing::is_negative(peek_root_scalar("numbersign"));
}
simdjson2_inline simdjson2_result<bool> value_iterator::is_integer() noexcept {
  return numberparsing::is_integer(peek_non_root_scalar("integer"));
}
simdjson2_inline simdjson2_result<number_type> value_iterator::get_number_type() noexcept {
  return numberparsing::get_number_type(peek_non_root_scalar("integer"));
}
simdjson2_inline simdjson2_result<number> value_iterator::get_number() noexcept {
  number num;
...
simdjson2_inline simdjson2_result<bool> value_iterator::is_root_integer(bool check_trailing) noexcept {
...
simdjson2_inline simdjson2_result<SIMDJSON2_IMPLEMENTATION::number_type> value_iterator::get_root_number_type(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
...
simdjson2_inline simdjson2_result<number> value_iterator::get_root_number(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
  auto json = peek_root_scalar("number");
  // Per https://www.exploringbinary.com/maximum-number-of-decimal-digits-in-binary-floating-point-numbers/,
  // 1074 is the maximum number of significant fractional digits. Add 8 more digits for the biggest
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<std::string_view> value_iterator::get_root_string(bool check_trailing, bool allow_replacement) noexcept {
  return get_root_raw_json_string(check_trailing).unescape(json_iter(), allow_replacement);
}
template <typename string_type>
simdjson2_warn_unused simdjson2_inline error_code value_iterator::get_root_string(string_type& receiver, bool check_trailing, bool allow_replacement) noexcept {
  std::string_view content;
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<std::string_view> value_iterator::get_root_wobbly_string(bool check_trailing) noexcept {
  return get_root_raw_json_string(check_trailing).unescape_wobbly(json_iter());
}
simdjson2_warn_unused simdjson2_inline simdjson2_result<raw_json_string> value_iterator::get_root_raw_json_string(bool check_trailing) noexcept {
  auto json = peek_scalar("string");
  if (*json != '"') { return incorrect_type_error("Not a string"); }
  if (check_trailing && !_json_iter->is_single_token()) { return TRAILING_CONTENT; }
  advance_scalar("string");
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<uint64_t> value_iterator::get_root_uint64(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<uint64_t> value_iterator::get_root_uint64_in_string(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
  auto json = peek_root_scalar("uint64");
  uint8_t tmpbuf[20+1+1]{}; // <20 digits> is the longest possible unsigned integer
  tmpbuf[20+1] = '\0'; // make sure that buffer is always null terminated.
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<int64_t> value_iterator::get_root_int64(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<int64_t> value_iterator::get_root_int64_in_string(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
  auto json = peek_root_scalar("int64");
  uint8_t tmpbuf[20+1+1]; // -<19 digits> is the longest possible integer
  tmpbuf[20+1] = '\0'; // make sure that buffer is always null terminated.
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<double> value_iterator::get_root_double(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<double> value_iterator::get_root_double_in_string(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
  auto json = peek_root_scalar("double");
  // Per https://www.exploringbinary.com/maximum-number-of-decimal-digits-in-binary-floating-point-numbers/,
  // 1074 is the maximum number of significant fractional digits. Add 8 more digits for the biggest
...
simdjson2_warn_unused simdjson2_inline simdjson2_result<bool> value_iterator::get_root_bool(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
...
simdjson2_inline simdjson2_result<bool> value_iterator::is_root_null(bool check_trailing) noexcept {
  auto max_len = peek_root_length();
...
simdjson2_warn_unused simdjson2_inline error_code value_iterator::skip_child() noexcept {
  SIMDJSON2_ASSUME( _json_iter->token._position > _start_position );
  SIMDJSON2_ASSUME( _json_iter->_depth >= _depth );

  return _json_iter->skip_child(depth());
}

simdjson2_inline value_iterator value_iterator::child() const noexcept {
  assert_at_child();
...
SIMDJSON2_PUSH_DISABLE_WARNINGS
SIMDJSON2_DISABLE_STRICT_OVERFLOW_WARNING
simdjson2_inline bool value_iterator::is_open() const noexcept {
  return _json_iter->depth() >= depth();
}
SIMDJSON2_POP_DISABLE_WARNINGS

simdjson2_inline bool value_iterator::at_end() const noexcept {
  return _json_iter->at_end();
}

simdjson2_inline bool value_iterator::at_start() const noexcept {
  return _json_iter->token.position() == start_position();
}

simdjson2_inline bool value_iterator::at_first_field() const noexcept {
  SIMDJSON2_ASSUME( _json_iter->token._position > _start_position );
  return _json_iter->token.position() == start_position() + 1;
}

simdjson2_inline void value_iterator::abandon() noexcept {
  _json_iter->abandon();
}

simdjson2_warn_unused simdjson2_inline depth_t value_iterator::depth() const noexcept {
  return _depth;
}
simdjson2_warn_unused simdjson2_inline error_code value_iterator::error() const noexcept {
  return _json_iter->error;
}
simdjson2_warn_unused simdjson2_inline uint8_t *&value_iterator::string_buf_loc() noexcept {
  return _json_iter->string_buf_loc();
}
simdjson2_warn_unused simdjson2_inline const json_iterator &value_iterator::json_iter() const noexcept {
  return *_json_iter;
}
simdjson2_warn_unused simdjson2_inline json_iterator &value_iterator::json_iter() noexcept {
  return *_json_iter;
}

simdjson2_inline const uint8_t *value_iterator::peek_start() const noexcept {
  return _json_iter->peek(start_position());
}
simdjson2_inline uint32_t value_iterator::peek_start_length() const noexcept {
  return _json_iter->peek_length(start_position());
}
simdjson2_inline uint32_t value_iterator::peek_root_length() const noexcept {
  return _json_iter->peek_root_length(start_position());
}

simdjson2_inline const uint8_t *value_iterator::peek_scalar(const char *type) noexcept {
...
simdjson2_inline void value_iterator::advance_scalar(const char *type) noexcept {
  logger::log_value(*_json_iter, start_position(), depth(), type);
  // If we're not at the position anymore, we don't want to advance the cursor.
...
simdjson2_inline error_code value_iterator::start_container(uint8_t start_char, const char *incorrect_type_message, const char *type) noexcept {
...
simdjson2_inline const uint8_t *value_iterator::peek_root_scalar(const char *type) noexcept {
  logger::log_value(*_json_iter, start_position(), depth(), type);
  if (!is_at_start()) { return peek_start(); }
...
simdjson2_inline const uint8_t *value_iterator::peek_non_root_scalar(const char *type) noexcept {
  logger::log_value(*_json_iter, start_position(), depth(), type);
  if (!is_at_start()) { return peek_start(); }
...
simdjson2_inline void value_iterator::advance_root_scalar(const char *type) noexcept {
  logger::log_value(*_json_iter, start_position(), depth(), type);
  if (!is_at_start()) { return; }
...
simdjson2_inline void value_iterator::advance_non_root_scalar(const char *type) noexcept {
  logger::log_value(*_json_iter, start_position(), depth(), type);
  if (!is_at_start()) { return; }
...
simdjson2_inline error_code value_iterator::incorrect_type_error(const char *message) const noexcept {
  logger::log_error(*_json_iter, start_position(), depth(), message);
  return INCORRECT_TYPE;
}

simdjson2_inline bool value_iterator::is_at_start() const noexcept {
  return position() == start_position();
}

simdjson2_inline bool value_iterator::is_at_key() const noexcept {
...
simdjson2_inline bool value_iterator::is_at_iterator_start() const noexcept {
...
inline void value_iterator::assert_at_start() const noexcept {
  SIMDJSON2_ASSUME( _json_iter->token._position == _start_position );
  SIMDJSON2_ASSUME( _json_iter->_depth == _depth );
  SIMDJSON2_ASSUME( _depth > 0 );
}

inline void value_iterator::assert_at_container_start() const noexcept {
  SIMDJSON2_ASSUME( _json_iter->token._position == _start_position + 1 );
  SIMDJSON2_ASSUME( _json_iter->_depth == _depth );
  SIMDJSON2_ASSUME( _depth > 0 );
}
...
simdjson2_inline void value_iterator::move_at_start() noexcept {
  _json_iter->_depth = _depth;
  _json_iter->token.set_position(_start_position);
}

simdjson2_inline void value_iterator::move_at_container_start() noexcept {
  _json_iter->_depth = _depth;
  _json_iter->token.set_position(_start_position + 1);
}

simdjson2_inline simdjson2_result<bool> value_iterator::reset_array() noexcept {
  if(error()) { return error(); }
  move_at_container_start();
  return started_array();
}

simdjson2_inline simdjson2_result<bool> value_iterator::reset_object() noexcept {
  if(error()) { return error(); }
  move_at_container_start();
  return started_object();
}
...
inline void value_iterator::assert_at_child() const noexcept {
  SIMDJSON2_ASSUME( _json_iter->token._position > _start_position );
  SIMDJSON2_ASSUME( _json_iter->_depth == _depth + 1 );
  SIMDJSON2_ASSUME( _depth > 0 );
}
...
inline void value_iterator::assert_is_valid() const noexcept {
  SIMDJSON2_ASSUME( _json_iter != nullptr );
}

simdjson2_inline bool value_iterator::is_valid() const noexcept {
  return _json_iter != nullptr;
}

simdjson2_inline simdjson2_result<json_type> value_iterator::type() const noexcept {
  switch (*peek_start()) {
    case '{':
...
simdjson2_inline token_position value_iterator::start_position() const noexcept {
  return _start_position;
}

simdjson2_inline token_position value_iterator::position() const noexcept {
  return _json_iter->position();
}

simdjson2_inline token_position value_iterator::end_position() const noexcept {
  return _json_iter->end_position();
}

simdjson2_inline token_position value_iterator::last_position() const noexcept {
  return _json_iter->last_position();
}

simdjson2_inline error_code value_iterator::report_error(error_code error, const char *message) noexcept {
...
simdjson2_inline simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::value_iterator>::simdjson2_result(SIMDJSON2_IMPLEMENTATION::ondemand::value_iterator &&value) noexcept
    : implementation_simdjson2_result_base<SIMDJSON2_IMPLEMENTATION::ondemand::value_iterator>(std::forward<SIMDJSON2_IMPLEMENTATION::ondemand::value_iterator>(value)) {}
simdjson2_inline simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::value_iterator>::simdjson2_result(error_code error) noexcept
    : implementation_simdjson2_result_base<SIMDJSON2_IMPLEMENTATION::ondemand::value_iterator>(error) {}

} // namespace simdjson2

#endif // SIMDJSON2_GENERIC_ONDEMAND_VALUE_ITERATOR_INL_H