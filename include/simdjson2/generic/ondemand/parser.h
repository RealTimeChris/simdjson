#ifndef SIMDJSON2_GENERIC_ONDEMAND_PARSER_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_ONDEMAND_PARSER_H
...
public:
   ...
   */
  inline explicit parser(size_t max_capacity = SIMDJSON2_MAXSIZE_BYTES) noexcept;

  inline parser(parser &&other) noexcept = default;
  simdjson2_inline parser(const parser &other) = delete;
  simdjson2_inline parser &operator=(const parser &other) = delete;
  simdjson2_inline parser &operator=(parser &&other) noexcept = default;
   ...
   * @param capacity The number of bytes allocated in the JSON (must be at least len+SIMDJSON2_PADDING).
   *
   * @return The document, or an error:
   *         - INSUFFICIENT_PADDING if the input has less than SIMDJSON2_PADDING extra bytes.
   ...
   */
  simdjson2_warn_unused simdjson2_result<document> iterate(padded_string_view json) & noexcept;
#ifdef SIMDJSON2_EXPERIMENTAL_ALLOW_INCOMPLETE_JSON
  simdjson2_warn_unused simdjson2_result<document> iterate_allow_incomplete_json(padded_string_view json) & noexcept;
#endif // SIMDJSON2_EXPERIMENTAL_ALLOW_INCOMPLETE_JSON
  /** @overload simdjson2_result<document> iterate(padded_string_view json) & noexcept */
  simdjson2_warn_unused simdjson2_result<document> iterate(const char *json, size_t len, size_t capacity) & noexcept;
  /** @overload simdjson2_result<document> iterate(padded_string_view json) & noexcept */
  simdjson2_warn_unused simdjson2_result<document> iterate(const uint8_t *json, size_t len, size_t capacity) & noexcept;
  /** @overload simdjson2_result<document> iterate(padded_string_view json) & noexcept */
  simdjson2_warn_unused simdjson2_result<document> iterate(std::string_view json, size_t capacity) & noexcept;
  /** @overload simdjson2_result<document> iterate(padded_string_view json) & noexcept */
  simdjson2_warn_unused simdjson2_result<document> iterate(const std::string &json) & noexcept;
  /** @overload simdjson2_result<document> iterate(padded_string_view json) & noexcept */
  simdjson2_warn_unused simdjson2_result<document> iterate(std::string &json) & noexcept;
  /** @overload simdjson2_result<document> iterate(padded_string_view json) & noexcept */
  simdjson2_warn_unused simdjson2_result<document> iterate(const simdjson2_result<padded_string> &json) & noexcept;
  /** @overload simdjson2_result<document> iterate(padded_string_view json) & noexcept */
  simdjson2_warn_unused simdjson2_result<document> iterate(const simdjson2_result<padded_string_view> &json) & noexcept;
  /** @overload simdjson2_result<document> iterate(padded_string_view json) & noexcept */
  simdjson2_warn_unused simdjson2_result<document> iterate(padded_string &&json) & noexcept = delete;
   ...
   *         - UNCLOSED_STRING if there is an unclosed string in the document.
   */
  simdjson2_warn_unused simdjson2_result<json_iterator> iterate_raw(padded_string_view json) & noexcept;
   ...
   */
  inline simdjson2_result<document_stream> iterate_many(const uint8_t *buf, size_t len, size_t batch_size = DEFAULT_BATCH_SIZE, bool allow_comma_separated = false) noexcept;
  /** @overload parse_many(const uint8_t *buf, size_t len, size_t batch_size) */
  inline simdjson2_result<document_stream> iterate_many(const char *buf, size_t len, size_t batch_size = DEFAULT_BATCH_SIZE, bool allow_comma_separated = false) noexcept;
  /** @overload parse_many(const uint8_t *buf, size_t len, size_t batch_size) */
  inline simdjson2_result<document_stream> iterate_many(const std::string &s, size_t batch_size = DEFAULT_BATCH_SIZE, bool allow_comma_separated = false) noexcept;
  inline simdjson2_result<document_stream> iterate_many(const std::string &&s, size_t batch_size, bool allow_comma_separated = false) = delete;// unsafe
  /** @overload parse_many(const uint8_t *buf, size_t len, size_t batch_size) */
  inline simdjson2_result<document_stream> iterate_many(const padded_string &s, size_t batch_size = DEFAULT_BATCH_SIZE, bool allow_comma_separated = false) noexcept;
  inline simdjson2_result<document_stream> iterate_many(const padded_string &&s, size_t batch_size, bool allow_comma_separated = false) = delete;// unsafe

  /** @private We do not want to allow implicit conversion from C string to std::string. */
  simdjson2_result<document_stream> iterate_many(const char *buf, size_t batch_size = DEFAULT_BATCH_SIZE) noexcept = delete;

  /** The capacity of this parser (the largest document it can process). */
  simdjson2_pure simdjson2_inline size_t capacity() const noexcept;
  /** The maximum capacity of this parser (the largest document it is allowed to process). */
  simdjson2_pure simdjson2_inline size_t max_capacity() const noexcept;
  simdjson2_inline void set_max_capacity(size_t max_capacity) noexcept;
  /**
   * The maximum depth of this parser (the most deeply nested objects and arrays it can process).
   * This parameter is only relevant when the macro SIMDJSON2_DEVELOPMENT_CHECKS is set to true.
   ...
   * and `max_depth` depth.
   *
   * The max_depth parameter is only relevant when the macro SIMDJSON2_DEVELOPMENT_CHECKS is set to true.
   * The document's instance current_depth() method should be used to monitor the parsing
   * depth and limit it if desired.
   *
   ...
   */
  simdjson2_warn_unused error_code allocate(size_t capacity, size_t max_depth=DEFAULT_MAX_DEPTH) noexcept;
   ...
   * as large as the input JSON plus SIMDJSON2_PADDING and then unescape all strings to this one buffer.
   *
   ...
   *            an additional SIMDJSON2_PADDING bytes.
   ...
   */
  simdjson2_inline simdjson2_result<std::string_view> unescape(raw_json_string in, uint8_t *&dst, bool allow_replacement = false) const noexcept;
   ...
   * @error STRING_ERROR if escapes are incorrect.
   */
  simdjson2_inline simdjson2_result<std::string_view> unescape_wobbly(raw_json_string in, uint8_t *&dst) const noexcept;
...
private:
  ...
  std::unique_ptr<simdjson2::internal::dom_parser_implementation> implementation{};
  size_t _capacity{0};
...
template<>
struct simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::parser> : public SIMDJSON2_IMPLEMENTATION::implementation_simdjson2_result_base<SIMDJSON2_IMPLEMENTATION::ondemand::parser> {
public:
  simdjson2_inline simdjson2_result(SIMDJSON2_IMPLEMENTATION::ondemand::parser &&value) noexcept; ///< @private
...
#endif // SIMDJSON2_GENERIC_ONDEMAND_PARSER_H