#ifndef SIMDJSON2_GENERIC_ONDEMAND_JSON_TYPE_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_ONDEMAND_JSON_TYPE_H
...
struct number {
   ...
   *    };
   */
  simdjson2_inline ondemand::number_type get_number_type() const noexcept;
  /**
   ...
   */
  simdjson2_inline uint64_t get_uint64() const noexcept;
  simdjson2_inline operator uint64_t() const noexcept;
   ...
   */
  simdjson2_inline int64_t get_int64() const noexcept;
  simdjson2_inline operator int64_t() const noexcept;
   ...
   */
  simdjson2_inline double get_double() const noexcept;
  simdjson2_inline operator double() const noexcept;
   ...
   */
  simdjson2_inline double as_double() const noexcept;
 ...
 * @param out The output stream.
 * @param type The json_type.
 * @throw simdjson2_error if the result being printed has an error. If there is an error with the
 *        underlying output stream, that error will be propagated (simdjson2_error will not be
 *        thrown).
 */
...
template<>
struct simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::json_type> : public SIMDJSON2_IMPLEMENTATION::implementation_simdjson2_result_base<SIMDJSON2_IMPLEMENTATION::ondemand::json_type> {
public:
  simdjson2_inline simdjson2_result(SIMDJSON2_IMPLEMENTATION::ondemand::json_type &&value) noexcept; ///< @private
...
#endif // SIMDJSON2_GENERIC_ONDEMAND_JSON_TYPE_H