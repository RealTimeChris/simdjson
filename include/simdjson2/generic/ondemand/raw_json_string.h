#ifndef SIMDJSON2_GENERIC_ONDEMAND_RAW_JSON_STRING_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_ONDEMAND_RAW_JSON_STRING_H
...
public:
   ...
   */
  simdjson2_inline raw_json_string() noexcept = default;
   ...
   */
  simdjson2_inline raw_json_string(const uint8_t * _buf) noexcept;
  /**
   ...
   */
  simdjson2_inline const char * raw() const noexcept;
   ...
   * for long strings.
   */
  simdjson2_inline bool unsafe_is_equal(size_t length, std::string_view target) const noexcept;
   ...
   */
  simdjson2_inline bool unsafe_is_equal(std::string_view target) const noexcept;
   ...
   *   s.unsafe_is_equal(target);
   */
  simdjson2_inline bool unsafe_is_equal(const char* target) const noexcept;

  /**
   * This compares the current instance to the std::string_view target: returns true if
   * they are byte-by-byte equal (no escaping is done).
   */
  simdjson2_inline bool is_equal(std::string_view target) const noexcept;

  /**
   * This compares the current instance to the C string target: returns true if
   * they are byte-by-byte equal (no escaping is done).
   */
  simdjson2_inline bool is_equal(const char* target) const noexcept;
   ...
   */
  static simdjson2_inline bool is_free_from_unescaped_quote(std::string_view target) noexcept;
  static simdjson2_inline bool is_free_from_unescaped_quote(const char* target) noexcept;
...
private:
   ...
   */
  simdjson2_inline void consume() noexcept { buf = nullptr; }

  /**
   * Checks whether the inner pointer is non-null and thus usable.
   */
  simdjson2_inline simdjson2_warn_unused bool alive() const noexcept { return buf != nullptr; }
   ...
   */
  simdjson2_inline simdjson2_warn_unused simdjson2_result<std::string_view> unescape(json_iterator &iter, bool allow_replacement) const noexcept;
   ...
   * @param iter A json_iterator, which contains a buffer where the string will be written.
   */
  simdjson2_inline simdjson2_warn_unused simdjson2_result<std::string_view> unescape_wobbly(json_iterator &iter) const noexcept;
  const uint8_t * buf{};
  friend class object;
  friend class field;
  friend class parser;
  friend struct simdjson2_result<raw_json_string>;
};

simdjson2_unused simdjson2_inline std::ostream &operator<<(std::ostream &, const raw_json_string &) noexcept;
...
template<>
struct simdjson2_result<SIMDJSON2_IMPLEMENTATION::ondemand::raw_json_string> : public SIMDJSON2_IMPLEMENTATION::implementation_simdjson2_result_base<SIMDJSON2_IMPLEMENTATION::ondemand::raw_json_string> {
public:
  simdjson2_inline simdjson2_result(SIMDJSON2_IMPLEMENTATION::ondemand::raw_json_string &&value) noexcept; ///< @private
  ...
  simdjson2_inline simdjson2_result<const char *> raw() const noexcept;
  simdjson2_inline simdjson2_warn_unused simdjson2_result<std::string_view> unescape(SIMDJSON2_IMPLEMENTATION::ondemand::json_iterator &iter, bool allow_replacement) const noexcept;
  simdjson2_inline simdjson2_warn_unused simdjson2_result<std::string_view> unescape_wobbly(SIMDJSON2_IMPLEMENTATION::ondemand::json_iterator &iter) const noexcept;
};

} // namespace simdjson2

#endif // SIMDJSON2_GENERIC_ONDEMAND_RAW_JSON_STRING_H