#ifndef SIMDJSON2_GENERIC_JSONCHARUTILS_H

#ifndef SIMDJSON2_CONDITIONAL_INCLUDE
#define SIMDJSON2_GENERIC_JSONCHARUTILS_H
...
// zero otherwise
simdjson2_inline uint32_t is_not_structural_or_whitespace(uint8_t c) {
  return internal::structural_or_whitespace_negated[c];
}

simdjson2_inline uint32_t is_structural_or_whitespace(uint8_t c) {
...
//
simdjson2_inline size_t codepoint_to_utf8(uint32_t cp, uint8_t *c) {
  if (cp <= 0x7F) {
    c[0] = uint8_t(cp);
    return 1; // ascii
  }
...
#if SIMDJSON2_IS_32BITS // _umul128 for x86, arm
// this is a slow emulation routine for 32-bit
//
static simdjson2_inline uint64_t __emulu(uint32_t x, uint32_t y) {
  return x * (uint64_t)y;
}
static simdjson2_inline uint64_t _umul128(uint64_t ab, uint64_t cd, uint64_t *hi) {
...
#endif // SIMDJSON2_GENERIC_JSONCHARUTILS_H