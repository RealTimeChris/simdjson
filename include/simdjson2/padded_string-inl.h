#ifndef SIMDJSON2_PADDED_STRING_INL_H
#define SIMDJSON2_PADDED_STRING_INL_H
...
// The allocate_padded_buffer function is a low-level function to allocate
// memory with padding so we can read past the "length" bytes safely. It is used
// by the padded_string class automatically. It returns nullptr in case of
// error: the caller should check for a null pointer. The length parameter is
// the maximum size in bytes of the string. The caller is responsible to free
// the memory (e.g., delete[] (...)).
inline char *allocate_padded_buffer(size_t length) noexcept {
  const size_t totalpaddedlength = length + SIMDJSON2_PADDING;
...
inline padded_string::padded_string(const std::string &str_) noexcept
    : viable_size(str_.size()),
      data_ptr(internal::allocate_padded_buffer(str_.size())) {
  if (data_ptr == nullptr) {
    viable_size = 0;
  } else {
...
inline padded_string::padded_string(std::string_view sv_) noexcept
    : viable_size(sv_.size()),
      data_ptr(internal::allocate_padded_buffer(sv_.size())) {
  if (simdjson2_unlikely(!data_ptr)) {
...
inline padded_string::~padded_string() noexcept { delete[] data_ptr; }
...
inline padded_string::operator padded_string_view() const noexcept {
  return padded_string_view(data(), length(), length() + SIMDJSON2_PADDING);
}

inline simdjson2_result<padded_string>
padded_string::load(std::string_view filename) noexcept {
  // Open the file
  SIMDJSON2_PUSH_DISABLE_WARNINGS
  SIMDJSON2_DISABLE_DEPRECATED_WARNING // Disable CRT_SECURE warning on MSVC:
