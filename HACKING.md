...
Here is wisdom about how to build, test and run simdjson2 from within the repository. This is mostly useful for people who plan to contribute simdjson2, or maybe study the design.

If you plan to contribute to simdjson2, please read our [CONTRIBUTING](https://github.com/simdjson2/simdjson2/blob/master/CONTRIBUTING.md) guide.

- [Hacking simdjson2](#hacking-simdjson2)
...
mkdir build
cd build
cmake -D SIMDJSON2_DEVELOPER_MODE=ON ..
cmake --build .
```
...
Build system targets that are only useful for developers of the simdjson2
library are behind the `SIMDJSON2_DEVELOPER_MODE` option. Enabling this option
makes tests, examples, benchmarks and other developer targets available. Not
enabling this option means that you are a consumer of simdjson2 and thus you
...
workflows used by simdjson2.

Directory Structure and Source
------------------------------

simdjson2's source structure, from the top level, looks like this:

* **CMakeLists.txt:** The main build system.
* **include:** User-facing declarations and inline definitions (most user-facing functions are inlined).
  * simdjson2.h: the `simdjson2` namespace. A "main include" that includes files from include/simdjson2/. This is equivalent to
    the distributed simdjson2.h.
    * simdjson2/*.h: Declarations for public simdjson2 classes and functions.
    * simdjson2/*-inl.h: Definitions for public simdjson2 classes and functions.
    * simdjson2/internal/*.h: the `simdjson2::internal` namespace. Private classes and functions used by the rest of simdjson2.
  * simdjson2/dom.h: the `simdjson2::dom` namespace. Includes all public DOM classes.
    * simdjson2/dom/*.h: Declarations/definitions for individual DOM classes.
  * simdjson2/arm64|fallback|haswell|icelake|ppc64|westmere.h: `simdjson2::<implementation>` namespace. Common implementation-specific tools like number and string parsing, as well as minification.
    * simdjson2/arm64|fallback|haswell|icelake|ppc64|westmere/*.h: implementation-specific functions such as , etc.
    * simdjson2/generic/*.h: the bulk of the actual code, written generically and compiled for each implementation, using functions defined in the implementation's .h files.
      * simdjson2/generic/dependencies.h: dependencies on common, non-implementation-specific simdjson2 classes. This will be included before including amalgamated.h.
      * simdjson2/generic/amalgamated.h: all generic ondemand classes for an implementation.
  * simdjson2/ondemand.h: the `simdjson2::ondemand` namespace. Includes all public ondemand classes.
    * simdjson2/builtin.h: the `simdjson2::builtin` namespace. Aliased to the most universal implementation available.
    * simdjson2/builtin/ondemand.h: the `simdjson2::builtin::ondemand` namespace.
    * simdjson2/arm64|fallback|haswell|icelake|ppc64|westmere/ondemand.h: the `simdjson2::<implementation>::ondemand` namespace. On-Demand compiled for the specific implementation.
    * simdjson2/generic/ondemand/*.h: individual On-Demand classes, generically written.
      * simdjson2/generic/ondemand/dependencies.h: dependencies on common, non-implementation-specific simdjson2 classes. This will be included before including amalgamated.h.
      * simdjson2/generic/ondemand/amalgamated.h: all generic ondemand classes for an implementation.
* **src:** The source files for non-inlined functionality (e.g. the architecture-specific parser
  implementations).
  * simdjson2.cpp: A "main source" that includes all implementation files from src/. This is
    equivalent to the distributed simdjson2.cpp.
  * *.cpp: other misc. implementations, such as `simdjson2::implementation` and the minifier.
  * arm64|fallback|haswell|icelake|ppc64|westmere.cpp: Architecture-specific parser implementations.
    * generic/*.h: `simdjson2::<implementation>` namespace. Generic implementation of the parser, particularly the `dom_parser_implementation`.
    * generic/stage1/*.h: `simdjson2::<implementation>::stage1` namespace. Generic implementation of the simd-heavy tokenizer/indexer pass of the simdjson2 parser. Used for the On-Demand interface
    * generic/stage2/*.h: `simdjson2::<implementation>::stage2` namespace. Generic implementation of the tape creator, which consumes the index from stage 1 and actually parses numbers and string and such. Used for the DOM interface.
...
* **singleheader:** Contains generated `simdjson2.h` and `simdjson2.cpp` that we release. The files `singleheader/simdjson2.h` and `singleheader/simdjson2.cpp` should never be edited by hand.
* **singleheader/amalgamate.py:** Generates `singleheader/simdjson2.h` and `singleheader/simdjson2.cpp` for release (python script).
...
* **tools:** Source for executables that can be distributed with simdjson2. Some examples:
...
While simdjson2 distributes just two files from the singleheader/ directory, we *maintain* the code in
multiple files under include/ and src/. The files include/simdjson2.h and src/simdjson2.cpp are the "spine" for
...
A key feature of simdjson2 is the ability to compile different processing kernels, optimized for specific instruction sets, and to select
the most appropriate kernel at runtime. This ensures that users get the very best performance while still enabling simdjson2 to run everywhere.
This technique is frequently called runtime dispatching. The simdjson2 achieves runtime dispatching entirely in C++: we do not assume
...
not use flags such as -mavx2, /arch:AVX2 and so forth while compiling simdjson2. When you do so, you allow the compiler to use advanced
...
So we compile simdjson2 for a generic processor. Our users should do the same if they want simdjson2's runtime dispatch to work. It is important
to understand that if runtime dispatching does not work, then simdjson2 will cause crashes on older processors. Of course, if a user chooses
...
In all cases, simdjson2 uses advanced instructions by relying on  "intrinsic functions": we do not write assembly code. The intrinsic functions
...
The simdjson2.h and simdjson2.cpp files in the singleheader directory are not always up-to-date with the rest of the code; they are only ever
...
The amalgamator script `amalgamate.py` generates singleheader/simdjson2.h by
reading through include/simdjson2.h, copy/pasting each header file into the amalgamated file at the
point it gets included (but only once per header). singleheader/simdjson2.cpp is generated from
src/simdjson2.cpp the same way, except files under generic/ may be included and copy/pasted multiple
times.
...
CMake will build a library. By default, it builds a static library (e.g., libsimdjson2.a on Linux).
...
mkdir buildshared
cd buildshared
cmake -D BUILD_SHARED_LIBS=ON -D SIMDJSON2_DEVELOPER_MODE=ON ..
cmake --build .
ctest
```
...
- Grab the simdjson2 code from GitHub, e.g., by cloning it using [GitHub Desktop](https://desktop.github.com/).
- Install [CMake](https://cmake.org/download/). When you install it, make sure to ask that `cmake` be made available from the command line. Please choose a recent version of cmake.
- Create a subdirectory within simdjson2, such as `build`.
...
- This last command (`cmake ...`) created a Visual Studio solution file in the newly created directory (e.g., `simdjson2.sln`). Open this file in Visual Studio. You should now be able to build the project and run the tests. For example, in the `Solution Explorer` window (available from the `View` menu), right-click `ALL_BUILD` and select `Build`. To test the code, still in the `Solution Explorer` window, select `RUN_TESTS` and select `Build`.


Though having Visual Studio installed is necessary, one can build simdjson2 using only cmake commands:
...
Furthermore, if you have installed LLVM clang on Windows, for example as a component of Visual Studio 2019, you can configure and build simdjson2 using LLVM clang on Windows using cmake:
