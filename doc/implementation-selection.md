...
The simdjson2 library takes advantage of SIMD instruction sets such as NEON, SSE and AVX to achieve
much of its speed. Because these instruction sets work differently, simdjson2 has to compile a
...
In many cases, you don't know where your compiled binary is going to run, so simdjson2 automatically
...
these implementations at compile time with `-DSIMDJSON2_IMPLEMENTATION_X=0` (where X is ICELAKE, HASWELL,
WESTMERE, ARM64, PPC64, LSX, LASX and FALLBACK).

The simdjson2 library automatically sets header flags for each implementation as it compiles; there
...
When you first use simdjson2, it will detect the CPU you're running on, and swap over to the fastest
...
```
When an implementation is not available, the bracket call `simdjson2::get_available_implementations()[name]`
...
If you're trying to do performance tests or see how different implementations of simdjson2 run, you
...
Furthermore, you should check that the implementation is available before setting it to `simdjson2::get_active_implementation()`
