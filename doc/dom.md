...
The simdjson2 library offers two distinct approaches on how to access a JSON document. We support
...
The simdjson2 library offers a simple DOM tree API, which you can access by creating a
...
Or by creating a padded string (for efficiency reasons, simdjson2 requires a string with
SIMDJSON2_PADDING bytes at the end) and calling `parse()`:

```c++
dom::parser parser;
dom::element doc = parser.parse("[1,2,3]"_padded); // parse a string, the _padded suffix creates a simdjson2::padded_string instance
```
...
During the`load` or `parse` calls, neither the input file nor the input string are ever modified. After calling `load` or `parse`, the source (either a file or a string) can be safely discarded. All of the JSON data is stored in the `parser` instance.  The parsed document is also immutable in simdjson2: you do not modify it by accessing it.

For best performance, a `parser` instance should be reused over several files: otherwise you will needlessly reallocate memory, an expensive process. It is also possible to avoid entirely memory allocations during parsing when using simdjson2. [See our performance notes for details](performance.md).
...
length of your input `l` in bytes. To see how to get the very best performance from a low-level approach, you way want to read our [performance notes](https://github.com/simdjson2/simdjson2/blob/master/doc/performance.md#padding-and-temporary-copies) on this topic (see the Padding and Temporary Copies section).
...
* **Extracting Values (with exceptions):** You can cast a JSON element to a native type: `double(element)` or
  ...
  dom::object and dom::array. An exception (`simdjson2::simdjson2_error`) is thrown if the cast is not possible.
...
* **Array Index:** To get at an array value by index, use the at() method: `array.at(0)` gets the
  ...
  > O(1) operation, which it is not presently in simdjson2. Instead, you should iterate over the elements
  > using a for-loop, as in our examples.
* **Array and Object size** Given an array or an object, you can get its size (number of elements or keys)
  with the `size()` method.
* **Checking an Element Type:** You can check an element's type with `element.type()`. It
  returns an `element_type` with values such as `simdjson2::dom::element_type::ARRAY`, `simdjson2::dom::element_type::OBJECT`, `simdjson2::dom::element_type::INT64`,  `simdjson2::dom::element_type::UINT64`,`simdjson2::dom::element_type::DOUBLE`, `simdjson2::dom::element_type::STRING`, `simdjson2::dom::element_type::BOOL` or, `simdjson2::dom::element_type::NULL_VALUE`.
* **Output to streams and strings:** Given a document or an element (or node) out of a JSON document, you can output a minified string version using the C++ stream idiom (`out << element`). You can also request the construction of a minified string version (`simdjson2::minify(element)`) or a prettified string version (`simdjson2::prettify(element)`). Numbers are serialized as 64-bit floating-point numbers (`double`).
...
While the simdjson2 library can be used in any project using C++ 11 and above, field iteration has special support C++ 17's destructuring syntax. For example:
...
The simdjson2 library also supports [JSON pointer](https://tools.ietf.org/html/rfc6901) through the
...
All simdjson2 APIs that can fail return `simdjson2_result<T>`, which is a &lt;value, error_code&gt;
...
int main(void) {
  simdjson2::dom::parser parser;
  simdjson2::dom::element tweets; // invalid until the get() succeeds
  auto error = parser.load("twitter.json").get(tweets);
  if (error) { std::cerr << error << std::endl; return EXIT_FAILURE; }

  simdjson2::dom::element res; // invalid until the get() succeeds
...
To ensure you don't write any code that uses exceptions, compile with `SIMDJSON2_EXCEPTIONS=OFF`. For example, if including the project via cmake:
...
If you're using simdjson2 to parse multiple documents, or in a loop, you should make a parser once
and reuse it. The simdjson2 library will allocate and retain internal buffers between parses, keeping
...
It's not just internal buffers though. The simdjson2 library reuses the document itself. The dom::element, dom::object and dom::array instances are *references* to the internal document.
You are only *borrowing* the document from simdjson2, which purposely reuses and overwrites it each
...
> **You are only borrowing the document from the simdjson2 parser. Don't keep it long term!**
...
The simdjson2 library automatically expands its memory capacity when larger documents are parsed, so
...
means that if you encounter a really, really large document, simdjson2 will not resize back down.
The simdjson2 library lets you adjust your allocation strategy to prevent your server from growing
without bound:
...
* You can set a *fixed capacity* that never grows, as well, which can be excellent for
  predictability and reliability, since simdjson2 will never call malloc after startup!
...
The simdjson2 API provides access to the JSON DOM (document-object-model) content as a tree of `dom::element` instances, each representing an object, an array or an atomic type (null, true, false, number). These `dom::element` instances are lightweight objects (e.g., spanning 16 bytes) and it might be advantageous to pass them by value, as opposed to passing them by reference or by pointer.

Padding and Temporary Copies
--------------

The simdjson2 function `parser.parse` reads data from a padded  buffer, containing SIMDJSON2_PADDING extra bytes added at the end.
...
memset(padded_json_copy.get() + json_len, 0, SIMDJSON2_PADDING);
simdjson2::dom::parser parser;
simdjson2::dom::element element = parser.parse(padded_json_copy.get(), json_len, false);
````

Setting the `realloc_if_needed` parameter `false` in this manner may lead to better performance since copies are avoided, but it requires that the user takes more responsibilities: the simdjson2 library cannot verify that the input buffer was padded with SIMDJSON2_PADDING extra bytes.

Performance Tips
---------------------

- For release builds, we recommend setting `NDEBUG` pre-processor directive when compiling the `simdjson2` library. Importantly, using the optimization flags `-O2` or `-O3` under GCC and LLVM clang does not set the `NDEBUG` directrive, you must set it manually (e.g., `-DNDEBUG`).
