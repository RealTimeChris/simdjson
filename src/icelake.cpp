...
alignas(32) uint64_t whitespace_pre[8];
  for (size_t x = 0; x < 4; ++x) {
  whitespace_pre[x] =
      in[x].eq({_mm512_shuffle_epi8(whitespace_table, in[x].chunks[0])});
  }
  const simd8<uint8_t> whitespace{simd::simd8<uint8_t>::load(whitespace_pre)};

  alignas(32) uint64_t op_pre[4];
  for (size_t x = 0; x < 4; ++x) {
    const simd8x64<uint8_t> curlified{in[x].chunks[0]};
    op_pre[x] = curlified.eq({_mm512_shuffle_epi8(op_table, in[x].chunks[0])});
  }
...
simdjson_warn_unused error_code implementation::minify(const uint8_t *buf, size_t len, uint8_t *dst, size_t &dst_len) const noexcept {
  return icelake::stage1::json_minifier::minify<256>(buf, len, dst, dst_len);
}

simdjson_warn_unused error_code dom_parser_implementation::stage1(const uint8_t *_buf, size_t _len, stage1_mode streaming) noexcept {
  this->buf = _buf;
  this->len = _len;
  return icelake::stage1::json_structural_indexer::index<256>(_buf, _len, *this, streaming);
}
