...
json_character_block::classify(const simd::simd8x64<uint8_t> (&in)[4]) {
  ...
  alignas(32) uint64_t whitespace_pre[2];
  for (size_t x = 0; x < 4; ++x) {
    whitespace_pre[x] =
        in[x].eq({_mm_shuffle_epi8(whitespace_table, in[x].chunks[0]),
                  _mm_shuffle_epi8(whitespace_table, in[x].chunks[1]),
                  _mm_shuffle_epi8(whitespace_table, in[x].chunks[2]),
                  _mm_shuffle_epi8(whitespace_table, in[x].chunks[3])});
  }
  ...
  for (size_t x = 0; x < 4; ++x) {
    const simd8x64<uint8_t> curlified{
        in[x].chunks[0] | 0x20, in[x].chunks[1] | 0x20, in[x].chunks[2] | 0x20,
        in[x].chunks[3] | 0x20};
    op_pre[x] = curlified.eq({_mm_shuffle_epi8(op_table, in[x].chunks[0]),
                              _mm_shuffle_epi8(op_table, in[x].chunks[1]),
                              _mm_shuffle_epi8(op_table, in[x].chunks[0]),
                              _mm_shuffle_epi8(op_table, in[x].chunks[1])});
  }
