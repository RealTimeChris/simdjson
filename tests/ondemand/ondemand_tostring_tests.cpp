 ...
 * https://github.com/simdjson2/simdjson2/issues/1768
...
bool issue1607() {
  ...
  std::string_view result = simdjson2::to_json_string(doc["test"]);
...
bool minify_demo() {
  ...
  std::cout << simdjson2::to_json_string(doc["test"]) << std::endl;
  TEST_SUCCEED();
}
...
bool load_to_string(const char *filename) {
  ...
  std::string_view serial1 = simdjson2::to_json_string(doc);
  error = parser.iterate(serial1, serial1.size() + simdjson2::SIMDJSON2_PADDING).get(doc);
  if (error) {
    std::cerr << error << std::endl;
    return false;
  }
  std::cout << "serializing twice." << std::endl;
  std::string_view serial2 = simdjson2::to_json_string(doc);
...
bool load_to_string_exceptionless(const char *filename) {
  ...
  error = simdjson2::to_json_string(doc).get(serial1);
  if (error) {
    std::cerr << error << std::endl;
    return false;
  }
  ...
  error = simdjson2::to_json_string(doc).get(serial2);
  if (error) {
    std::cerr << error << std::endl;
    return false;
  }
...
bool empty_object() {
  ...
  ASSERT_SUCCESS( simdjson2::to_json_string(doc).get(serial));
...
bool at_start_array() {
  ...
  std::string_view serial;
  ASSERT_SUCCESS( simdjson2::to_json_string(array).get(serial));
...
bool at_start_object() {
  ...
  std::string_view serial;
  ASSERT_SUCCESS( simdjson2::to_json_string(object).get(serial));
...
bool complex_case() {
  ...
  ASSERT_SUCCESS( simdjson2::to_json_string(obj).get(serial));
